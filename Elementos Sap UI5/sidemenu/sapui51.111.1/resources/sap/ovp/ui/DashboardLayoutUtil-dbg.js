sap.ui.define([
    ...
    var oLogger = new OVPLogger("OVP.ui.DashboardLayoutUtil");
    var DashboardLayoutUtil = function (uiModel) {
        ...
        this.changedCards = {};
        this.isRTLEnabled = Core.getConfiguration().getRTL();
        switch (true) {
    ...
    DashboardLayoutUtil.prototype.setLayout = function (layout) {
        ...
        var iMainComponentIdEndIndex = this.layoutDomId.indexOf("---") + 3;
        var sComponentId = this.layoutDomId.substring(0, iMainComponentIdEndIndex);
        var sMainViewId = this.layoutDomId.substring(iMainComponentIdEndIndex);
        this.componentDomId = sComponentId + sMainViewId.split("--")[0];
    };
    ...
    DashboardLayoutUtil.prototype.updateLayoutData = function (iDashboardWidth) {
        ...
        jQuery(".ovpWrapper .sapFDynamicPageTitle").css({ visibility: "visible" });
    ...
    DashboardLayoutUtil.prototype.getRearrange = function (settings) {
        ...
        return new Rearrange(merge(defaultSettings, settings));
    };
    ...
    DashboardLayoutUtil.prototype.resizeLayout = function (iWidth) {
        ...
        /*
         *  If iWidth is 0, then there is no need to recalculate the card positions.
         *  recalculation triggers a flicker in layout with keep-alive set to true when iWidth is 0
         */
        if (this.oLayoutData.layoutWidthPx !== iWidth && iWidth !== 0) {
            this.updateLayoutData(iWidth);
            bTriggerResize =
                Math.abs(this.lastTriggeredColWidth - this.oLayoutData.colWidthPx) > this.convertRemToPx(0.5);
            ...
            for (var i = 0; i < this.aCards.length; i++) {
                ...
                if (element) {
                    ...
                    element.style[this.cssVendorTransform] =
                        "translate3d(" + oCard.dashboardLayout.left + " ," + oCard.dashboardLayout.top + ", 0px)";
     ...
     */
    DashboardLayoutUtil.prototype.getCardId = function (sCardDomId) {
        // <mainComponentId>---mainView--ovpLayout--card00 --> card00
        var iDomIdIndex = sCardDomId.indexOf("---") + 3;
        var sMainViewId = sCardDomId.substring(iDomIdIndex);
        return sMainViewId ? sMainViewId.split("--")[2] : "";
    };
     ...
     * @returns {String}  manifest id of the card
     */
    DashboardLayoutUtil.prototype.getCardIdFromComponent = function (sCardComponentId) {
        // <mainComponentId>---mainView--card00 --> card00
        var iDomIdIndex = sCardComponentId.indexOf("---") + 3;
        var sLayoutId = sCardComponentId.substring(iDomIdIndex);
        return sLayoutId ? sLayoutId.split("--")[1] : "";
    };
    ...
    DashboardLayoutUtil.prototype.setAutoCardSpanHeight = function (evt, cardId, height) {
        ...
        if (!cardId && evt && evt.target.parentElement) {
            var sCardDomId = evt.target.parentElement.parentElement.id;
            var iDomIdIndex = sCardDomId.indexOf("---") + 3;
            var sMainViewId = sCardDomId.substring(iDomIdIndex);
            cardId = sMainViewId.split("--")[1];
        }
        var iHeight = height;
        if (!iHeight && evt) {
        ...
        if (this.isCardAutoSpan(cardId)) {
            ...
            layoutChanges = this.dashboardLayoutModel.resizeCard(
                cardId,
                {
                    rowSpan: iRows,
                    colSpan: 1
    ...
    DashboardLayoutUtil.prototype.calculateCardProperties = function (sCardId) {
        ...
        if (oGenCardCtrl) {
            ...
            if (oCard.template === "sap.ovp.cards.list" || oCard.template === "sap.ovp.cards.v4.list") {
            ...
            } else if (oCard.template === "sap.ovp.cards.table" || oCard.template === "sap.ovp.cards.v4.table") {
                ...
                if (oCard.settings.listFlavor === "carousel") {
                    minCardHeight =
                        iHeaderHeight +
                        jUtils.getOuterHeight(document.querySelector(".sapOvpCarouselContentHeader")) +
                        240 +
                        jUtils.getOuterHeight(document.querySelector(".sapMCrslControlsBottom.sapMCrslControls"));
                } else {
                    ...
                    var densityType = oGenCardCtrl
                        .getView()
                        .getModel("ovpCardProperties")
                        .getProperty("/densityStyle");
                    if (densityType === "cozy") {
    ...
    DashboardLayoutUtil.prototype._sizeCard = function (oCard) {
        ...
        var iContainerHeight = this.dashboardLayoutModel._findHighestOccupiedRow() + 32;
        jQuery(".sapUshellEasyScanLayoutInner").css({ height: iContainerHeight + "px" });
        jQuery(".sapUshellEasyScanLayout").css({ height: iContainerHeight + "px" });
    };
    ...
    DashboardLayoutUtil.prototype._triggerCardResize = function (oCard) {
        ...
        try {
            if ((cardLayout.autoSpan || !cardLayout.visible) && oGenCardCtrl) {
                ...
                if (oCardBinding && cardSizeProperties) {
                    ...
                    var itemLength = iNoOfItems
                        ? Math.min(iNoOfItems, oCard.dashboardLayout.noOfItems)
                        : oCard.dashboardLayout.noOfItems;
                    ...
                    if (oCard.template === "sap.ovp.cards.table" || oCard.template === "sap.ovp.cards.v4.table") {
                        ...
                        iContainerHeight =
                            iActualNoOfItems * cardSizeProperties.itemHeight + cardSizeProperties.dropDownHeight;
                        $card.style.height =
                            Math.min(iContainerHeight, iAvailableSpace) + oCard.dashboardLayout.headerHeight + "px";
                    }
                }
                return;
            }
        } catch (error) {
            oLogger.warning("Card auto span failed for card " + cardId + " and error is  " + error.toString());
        }
        ...
        try {
            if (oGenCardCtrl) {
                ...
                oLogger.warning("OVP resize: no controller found for " + cardId);
            }
        } catch (err) {
            oLogger.warning("OVP resize: " + cardId + " catch " + err.toString());
        }
    };
        ...
        aCards.forEach(
            function (oCardObj) {
                ...
                if (element) {
                    ...
                    bSideCard =
                        oCardObj.dashboardLayout.column + oCardObj.dashboardLayout.colSpan ===
                        this.oLayoutData.colCount + 1;
                    if (bSideCard) {
                    ...
                    if (
                        ((oCardObj.template === "sap.ovp.cards.list" ||
                            oCardObj.template === "sap.ovp.cards.v4.list") &&
                            oCardObj.dashboardLayout.colSpan === 2) ||
                        ((oCardObj.template === "sap.ovp.cards.linklist" ||
                            oCardObj.template === "sap.ovp.cards.v4.linklist") &&
                            oCardObj.settings.listFlavor === "carousel" &&
                            oCardObj.dashboardLayout.colSpan === 3)
                    ) {
                        element.classList.add("sapOvpNotResizableRight");
                    }
                    if (
                        (oCardObj.template === "sap.ovp.cards.linklist" ||
                            oCardObj.template === "sap.ovp.cards.v4.linklist") &&
                        oCardObj.settings.listFlavor === "carousel" &&
                        oCardObj.dashboardLayout.rowSpan === 45
                    ) {
    ...
    DashboardLayoutUtil.prototype.updateCardSize = function (sCardId, ghostHeight, ghostWidth) {
        ...
        if (
            ...
            oCardController
                .getView()
                .byId("pictureCarousel")
                .$()
                .css("height", ghostHeight - (oCard.dashboardLayout.headerHeight + 2 * this.CARD_BORDER_PX) + "px");
        }
    };

    DashboardLayoutUtil.prototype.resizeCard = function (cardDomId, span, aDragOrResizeChange) {
        this.changedCards = this.dashboardLayoutModel.resizeCard(
            this.getCardId(cardDomId),
            span,
                /*manual resize*/ true,
            aDragOrResizeChange
        );
    ...
    DashboardLayoutUtil.prototype._getCardController = function (cardId) {
        var oCtrl = null;
        var oComponent = Core.byId(this._getCardComponentDomId(cardId));
        if (oComponent) {
    ...
    DashboardLayoutUtil.prototype.setAriaPos = function () {
        var cardsOnColumn = [];
        this.ariaPos = {};
        this.aCards.forEach(function (card) {
            if (card.dashboardLayout.visible === true) {
                card.visited = false;
                if (cardsOnColumn[card.dashboardLayout.column - 1]) {
                    cardsOnColumn[card.dashboardLayout.column - 1].push(card);
                } else {
                    cardsOnColumn[card.dashboardLayout.column - 1] = [card];
                }
            }
        });
        cardsOnColumn.forEach(function (cardsArray) {
        ...
        cardPositionHelper.setAriaPosition(cardsOnColumn, this);
    };
