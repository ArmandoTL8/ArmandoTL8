...
sap.ui.define([
    ...
    var oLogger = new OVPLogger("OVP.ui.ComponentContainerDesigntimeMetadata");
    ...
    var oElementMover = new ElementMover(),
        ...
        _iterateOverAllSiblings = function (oSelectedElement, fnStep) {
            var oOverlay = OverlayRegistry.getOverlay(oSelectedElement),
                oParentOverlay = oOverlay && oOverlay.getParentElementOverlay(),
                aSiblingOverlays =
                    oParentOverlay && OverlayUtil.findAllSiblingOverlaysInContainer(oOverlay, oParentOverlay);
    ...
    ElementMover.prototype.checkTargetZone = function (oAggregationOverlay, oOverlay, bOverlayNotInDom) {
        ...
        var vTargetRelevantContainerAfterMove = MetadataPropagationUtil.getRelevantContainerForPropagation(
            oAggregationDtMetadata.getData(),
            oMovedElement
        );
        vTargetRelevantContainerAfterMove = vTargetRelevantContainerAfterMove
        ...
        if (oMovedOverlay.getParent().getElement() !== oTargetElement) {
            ...
            if (
                Object.keys(aBindings).length > 0 &&
                oMovedElement.getBindingContext() &&
                oTargetElement.getBindingContext()
            ) {
            ...
            reveal: {
                ...
                getLabel: function (oControl) {
                    ...
                    if (oComponentData) {
                        ...
                        oLogger.error("Card id " + sCardId + " is not present in the manifest");
                        return "";
                    }
                }.bind(oAppMain)
            }
    ...
    oComponentContainerDesigntimeMetadata.actions["settings"] = function (oElement) {
        if (!oElement.getComponentInstance()) {
            //Adding this check to skip card containers that don't have card components. Case of hidden cards in DTA
            return {};
        }
                ...
                handler: function (oSelectedElement, fGetUnsavedChanges) {
                    ...
                    if (oCutOverlay) {
                        var oMovedElement = oCutOverlay.getElement(),
                            ...
                            oMainComponent = oSelectedElement
                        ...
                        if (oUIModel.getProperty("/containerLayout") === "resizable") {
                            ...
                            if (
                                oTargetCardObj.dashboardLayout.column + oSourceCardObj.dashboardLayout.colSpan >
                                iColumnCount + 1
                            ) {
                                oPayLoadData.dashboardLayout[sLayoutKey].colSpan =
                                    oTargetCardObj.dashboardLayout.colSpan;
                                oPayLoadData.dashboardLayout[sLayoutKey].oldColSpan =
                                    oSourceCardObj.dashboardLayout.colSpan;
                                oPayLoadData.dashboardLayout[sLayoutKey].rowSpan =
                                    oSourceCardObj.dashboardLayout.rowSpan;
                                oPayLoadData.dashboardLayout[sLayoutKey].oldRowSpan =
                                    oSourceCardObj.dashboardLayout.rowSpan;
                            }
                            aChanges.push({
        ...
        oSettings["RemoveCard"] = {
            ...
            isEnabled: function (oSelectedElement) {
                ...
                return (
                    oSelectedElement.getId().indexOf(CommonUtils._getLayerNamespace() + ".") !== -1 &&
                    !oSettings.cloneCard &&
                    !oSettings.newCard
                );
            },
