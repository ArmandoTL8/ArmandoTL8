...
sap.ui.define([
    ...
    var oAppComponent = {};
    var oLogger = new OVPLogger("OVP.lib.CommonMethods");
    ...
    function fnPublishEvent(sChannel, sEvent, oData) {
        ...
        Core.getEventBus().publish(sChannel, sEvent, oData);
    }
    ...
    function fnGetApplicationName(sURL) {
        ...
        if (aParts.length < 2) {
            ...
            if (
                iPosTilde &&
                iPosTilde !== -1 &&
                (iPosTilde < iPosQuestion || iPosQuestion === -1) &&
                (iPosTilde < iPosSlash || iPosSlash === -1) &&
                (iPosTilde < iPosAmp || iPosAmp === -1)
            ) {
                return sHash.substr(0, iPosTilde);
            } else if (
                iPosQuestion &&
                iPosQuestion !== -1 &&
                (iPosQuestion < iPosTilde || iPosTilde === -1) &&
                (iPosQuestion < iPosSlash || iPosSlash === -1) &&
                (iPosQuestion < iPosAmp || iPosAmp === -1)
            ) {
                return sHash.substr(0, iPosQuestion);
            } else if (
                iPosSlash &&
                iPosSlash !== -1 &&
                (iPosSlash < iPosTilde || iPosTilde === -1) &&
                (iPosSlash < iPosQuestion || iPosQuestion === -1) &&
                (iPosSlash < iPosAmp || iPosAmp === -1)
            ) {
                return sHash.substr(0, iPosSlash);
            } else if (
                iPosAmp &&
                iPosAmp !== -1 &&
                (iPosAmp < iPosTilde || iPosTilde === -1) &&
                (iPosAmp < iPosQuestion || iPosQuestion === -1) &&
                (iPosAmp < iPosSlash || iPosSlash === -1)
            ) {
