sap.ui.define([
    ...
    return Controller.extend("sap.ovp.app.Main", {
        ...
        onInit: function () {
            ...
            this.globalEventBus = Core.getEventBus();
        ...
        recreateCard: function (oCardProperties) {
            ...
            if (this.getView()) {
                this.getView().getModel("ui").setProperty("/bEnableResetBtn", true);
            }
        },
        ...
        _getLibraryResourceBundle: function () {
            if (!this.oLibraryResourceBundle) {
                this.oLibraryResourceBundle = Core.getLibraryResourceBundle("sap.ovp");
            }
        ...
        _resetDashboardLayout: function () {
            if (this.getLayout().getMetadata().getName() === "sap.ovp.ui.DashboardLayout") {
                if (this.getLayout().getDashboardLayoutUtil()) {
                    ...
                    this.getView().getModel("ui").setProperty("/bEnableResetBtn", false);
                }
        ...
        _fnIsLoadDataOnGoButton: function (bGlobalFilterMandatoryItemsFilled) {
            ...
            var dataLoadSetting = this.getUIModel().getProperty("/dataLoadSettings");
            var sLoadBehaviour = dataLoadSetting ? dataLoadSetting.loadDataOnAppLaunch : "ifAnyFilterExist";
        ...
        onTriggerFilterSearch: function () {
            if (this.getGlobalFilter() && this.oGlobalFilter.validateMandatoryFields()) {
        ...
        _initGlobalFilter: function () {
            ...
            this.oGlobalFilterLoadedPromise = new Promise(
                function (resolve, reject) {
                    ...
                    oGlobalFilter._oSearchButton &&
                        oGlobalFilter._oSearchButton.attachPress(
                            function () {
        ...
        _getEntityRelevantFilters: function (oEntityType, aFilters, oEntityModel, oFilterModel) {
            ...
            if (aFilters.length > 0 && oEntityType) {
                var entityType = this.isMacroFilterBar ? oEntityType.entityType : this.getGlobalFilter().getEntityType();
        ...
        _checkIsCardValid: function (sCardTemplate) {
            ...
            if (
                (oComponentMetadata = oComponentClass.getMetadata()) &&
                (oCustomizations = oComponentMetadata.getCustomizing())
            ) {
        ...
        _onManageCardsMenuButtonPress: function () {
            ...
            if (this.manageCardsDialog) {
                this.openManageCardsDialog(aOrderedCards);
            } else {
                Fragment.load({
                    ...
                    controller: {
                            ...
                            MessageBox.show(OvpResources.getText("reset_cards_confirmation_msg"), {
                                ...
                                onClose: function (oAction) {
                                    if (oAction === MessageBox.Action.OK) {
                                        ...
                                        for (var i = 0; i < aOrderedCards.length; i++) {
                                            aCardForDeletion.push(this.getView().byId(aOrderedCards[i].id));
                                        }
                        ...
                        afterManageCardsDialogOpen: function () {
                            ...
                            var bEnableResetBtn = this.getView().getModel("ui").getProperty("/bEnableResetBtn");
                            resetButton.setEnabled( !!bEnableResetBtn ); 
                    ...
                    function (oControl) {
                        this.manageCardsDialog = oControl;
         ...
         * @param variantManagementSaved  :  Boolean flag to prevent custom filters being set again if triggered by varaint management save
         * @returns {object}
         * @private
         */
        _getCurrentAppState: function (variantManagementSaved) {
            var oGlobalFilter = this.getGlobalFilter();
         ...
         * @param variantManagementSaved    :  Boolean flag to prevent custom filters being set if triggered by varainatmanegment save
         * @private
         */
        _storeCurrentAppStateAndAdjustURL: function (oEvent, variantManagementSaved) {
            ...
            this.oAppStatePromise = new Promise(
                function (resolve, reject) {
                    this.rejectPreviousPromise = reject;
                    var oAppInnerData = this._getCurrentAppState(variantManagementSaved);
                    var oCurrentAppState = this.oNavigationHandler.storeInnerAppStateWithImmediateReturn(oAppInnerData, true);
                    // oCurrentAppState contains a promise object that resolves when appstate is created successfully
                    oCurrentAppState.promise.then(
        ...
        onMacroFilterBarSearch: function (oEvent) {
            ...
            this._clearStoredEntities(); //Clear previously temporarily stored entity types
            this.aFilters = oEvent.getParameter("filters");
            var sBatchGroupId = "ovp-" + new Date().getTime();
            for (var modelKey in this.oCardsModels) {
                if (this.oCardsModels.hasOwnProperty(modelKey) && !CommonUtils.isODataV4(this.oCardsModels[modelKey])) {
                    try {
