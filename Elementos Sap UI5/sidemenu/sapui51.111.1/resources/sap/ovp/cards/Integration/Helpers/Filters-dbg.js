sap.ui.define([
...
], function (MetadataAnalyser, CommonUtils, SelectionVariant, AnnotationHelper, i18nHelper) {
    "use strict";
    ...
    function getParameterizedEntityProperties(oModel, oEntitySet, sProperty, oCardDefinition) {
        ...
        if (bParameterised) {
            aProperties = AnnotationHelper.resolveAnalyticalParameterizedEntitySet(oModel, oEntitySet, oSelectionVariant, aParameters, oGlobalFilter, true) || [];
        } else {
    ...
    */
     ...
     * @param {object} oCardDefinition Card defination object
     * @param {string} sPropertyName 
     * @returns {boolean} true if the semantic date range property exists in ovp generated properties false o/w.
    */
    function IsSemanticDateRangeValid(oCardDefinition, oProperty) {
        ...
        if (aDateProperties && aDateProperties.length && sPropertyName) {
        ...
        } else if (CommonUtils.isDate(oProperty)) {
            ...
            return oOvpConfig && oOvpConfig.useDateRangeType;
        }
    }
    ...
    */
    function getDateRangeDefaultValue(oCardDefinition, sPropertyName) {
        ...
        if (oDateProperties && oDateProperties[sPropertyName] && oDateProperties[sPropertyName]['defaultValue']) {
            ...
            var oRawManifest = oCardDefinition.cardComponentData.appComponent.getManifestObject().getRawJson();
            var oOvpConfig = oRawManifest && oRawManifest["sap.ovp"];
            var oFields = oOvpConfig.filterSettings && oOvpConfig.filterSettings.dateSettings && oOvpConfig.filterSettings.dateSettings.fields;
    ...
    */
    ...
    /**
     * Returns the value for semantic date range control
     * 
     * @param {string} sParameterName Property name.
     * @param {object} oGlobalFilter
     * @returns {object} Object consists of operator and values.
    */
    function getDateRangeControlValue(sParameterName, oGlobalFilter) {
        var aFilterItems = oGlobalFilter && oGlobalFilter.getAllFilterItems();

        var aRelatedFilterItem = aFilterItems.filter(function (oFilterItem) {
        ...
        var oDateRangeControl = aRelatedFilterItem[0] && aRelatedFilterItem[0].getControl();
