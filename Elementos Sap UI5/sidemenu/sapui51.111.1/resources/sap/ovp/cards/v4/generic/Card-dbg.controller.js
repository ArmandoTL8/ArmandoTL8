sap.ui.define([
    ...
    return Controller.extend("sap.ovp.cards.v4.generic.Card", {
        ...
        onAfterRendering: function () {
            ...
            var oHeaderTitle = this.getView().byId("ovpHeaderTitle"),
                oQuickViewCardHeaderEnabled = this.getView().byId("ovpQuickviewCardHeader");
            if (oCardHeaderDomRef && !oCardHeaderDomRef.getAttribute("ariaLabelledBy")) {
                if (oHeaderTitle && oHeaderTitle.getText()) {
                    ...
                    var sQuickViewCardHeaderID = oQuickViewCardHeaderEnabled.getDomRef().getAttribute("id");
                        ...
                        if (oUiModel.getProperty("/containerLayout") === "resizable") {
                            ...
                            if (oDashboardLayoutUtil) {
                                ...
                                if (oDashboardLayoutUtil.isCardAutoSpan(oCompData.cardId)) {
                                    this.resizeHandlerId = ResizeHandler.register(
                                        this.getView(),
                                        function (oEvent) {
                                            oLogger.info(
                                                "DashboardLayout autoSize:" +
                                                oEvent.target.id +
                                                " -> " +
                                                oEvent.size.height
                                            );
            ...
            if (oCompData && oCompData.mainComponent) {
                ...
                if (oMainComponent.bGlobalFilterLoaded) {
                    bIsNavigable = !this.checkHeaderNavForChart() && this.checkNavigation();
                }
            ...
            if (dropDown) {
                ...
                if (this.getView().getModel("ui").getProperty("/bSelectionChanged")) {
                        var sDropdownElement = document.getElementById(oDropDown.sId);
                            sDropdownElement && sDropdownElement.focus();
                            this.getView().getModel("ui").setProperty("/bSelectionChanged", false);
                    }
            }
        ...
        checkNavigation: function () {
            ...
            if (oEntityType) {
                if (oCardPropsModel) {
                    var sIdentificationAnnotationPath = oCardPropsModel.getProperty(
                    ...
                    if (
                        sContentFragment &&
                        (sContentFragment === "sap.ovp.cards.stack.Stack" ||
                            sContentFragment === "sap.ovp.cards.quickview.Quickview")
                    ) {
                        var aAnnotationPath = sIdentificationAnnotationPath
                            ? sIdentificationAnnotationPath.split(",")
                            : [];
                    ...
                    if (
                        oCardPropsModel &&
                        oCardPropsModel.getProperty("/template") === "sap.ovp.cards.charts.analytical"
                    ) {
                        ...
                        if (oEntityType && sKpiAnnotationPath) {
                            ...
                            if (oRecord && oRecord.Detail) {
                                var sSemanticObject =
                                    oRecord.Detail.SemanticObject && oRecord.Detail.SemanticObject.String;
                ...
                oCardPropsModel &&
                oCardPropsModel.getProperty("/template") === "sap.ovp.cards.v4.linklist" &&
        ...
        checkNavigationForLinkedList: function () {
            if (this.getEntityType()) {
                ...
                if (
                    oLineItemRecords &&
                    (oLineItemRecords[0].RecordType === "com.sap.vocabularies.UI.v1.DataFieldForAction" ||
        ...
        isNavigationInAnnotation: function (aRecords) {
            if (aRecords && aRecords.length) {
                for (var i = 0; i < aRecords.length; i++) {
                    ...
                    if (
                        oItem.$Type === "com.sap.vocabularies.UI.v1.DataFieldForIntentBasedNavigation" ||
                        oItem.$Type === "com.sap.vocabularies.UI.v1.DataFieldForAction" ||
                        oItem.$Type === "com.sap.vocabularies.UI.v1.DataFieldWithUrl"
                    ) {
                        return 1;
                    }
                }
        ...
        checkAPINavigation: function () {
            var oComponentData = this.getOwnerComponent().getComponentData(),
                bCheckIfValidArg = oComponentData.fnCheckNavigation &&
                    typeof oComponentData.fnCheckNavigation === "function",
            ...
            if (fnCheckNavigation) {
                ...
                if (!this.checkHeaderNavForChart() && this.checkNavigation()) {
                    return true;
                }
            }
        ...
        onHeaderClick: function (oEvent) {
            ...
            var sNavMode = oEvent.ctrlKey || oEvent.metaKey
        ...
        checkHeaderNavForChart: function () {
            ...
            if (
                ...
                (template == "sap.ovp.cards.charts.analytical" || "sap.ovp.cards.charts.smart.chart")
            ) {
                return true;
            } else {
                return false;
        ...
        setHeaderCounter: function (oItemsBinding, countHeader) {
            var iTotal = oItemsBinding.getLength();
            var iCurrent = oItemsBinding.getCurrentContexts().length;
            var oCard,
                countHeaderText = "";
                        ...
                        function (oEvent) {
                            ...
                            if (kpiHeader[0]) {
                                ...
                                if (iTotal === 0) {
                                    ...
                                    this._setSubTitleWithUnitOfMeasure(oDataSet);
         ...
         */
        _setSubTitleWithUnitOfMeasure: function (oDataSet) {
            var oCardPropertiesModel = this.getCardPropertiesModel();
        ...
        getEntityNavigationEntries: function (oContext, sAnnotationPath) {
            ...
            if (Array.isArray(aRecords)) {
                ...
                for (var i = 0; i < aRecords.length; i++) {
                    if (aRecords[i].$Type === "com.sap.vocabularies.UI.v1.DataFieldForIntentBasedNavigation") {
                        aNavigationFields.push({
                            type: aRecords[i].$Type,
                            semanticObject: aRecords[i].SemanticObject,
                            action: aRecords[i].Action,
                            label: aRecords[i].Label ? aRecords[i].Label : null
                        });
                    }
                    if (
                        aRecords[i].$Type === "com.sap.vocabularies.UI.v1.DataFieldWithUrl" &&
                        !aRecords[i].Url.UrlRef
                    ) {
                        var oModel = this.getView().getModel();
                        var oMetaModel = oModel.getMetaModel();
                        var sPath;
                        if (oContext) {
                            sPath = oContext.getInterface(0).getPath();
                        } else {
                            sPath = "/" + oCardPropsModel.getInterface().getData().entitySet;
                        }
                        var oEntityBindingContext = oMetaModel.createBindingContext(sPath);
                        var sBindingString = oDataAnnotationHelperV4.format(aRecords[i].Url, {
        ...
        _processCustomParameters: function (oContextData, oSelectionData, oContextParameters) {
            ...
            if (
                !sCustomParams ||
                !this.oMainComponent.templateBaseExtension.provideCustomParameter ||
                !this.oMainComponent.onCustomParams
            ) {
                return;
            }
            ...
            var fnGetParameters = this.oMainComponent.templateBaseExtension.provideCustomParameter(sCustomParams)
                ? this.oMainComponent.templateBaseExtension.provideCustomParameter(sCustomParams)
                : this.oMainComponent.onCustomParams(sCustomParams);
            ...
            var bIgnoreEmptyString =
                bIsObject && (oCustomParams.bIgnoreEmptyString || oCustomParams.ignoreEmptyString);
        ...
        _getEntityNavigationParameters: function (oEntity, oCustomParameters, oContext) {
            ...
            if (!oStaticLinkList) {
                ...
                aCardParameters &&
                    aCardParameters.forEach(function (oCardParameter) {
                ...
                if (oEntity) {
                    var key,
                        oEntityTypePropertyKeys =
                            oEntityType && oEntityType.property && Object.keys(oEntityType.property);
                    for (var i = 0; i < oEntityTypePropertyKeys.length; i++) {
                        if (oEntityTypePropertyKeys[i].startsWith("$")) {
                            continue;
                        }
                        key = oEntityTypePropertyKeys[i];
                ...
                var sKpiAnnotationPath =
                    oCardPropertiesModel && oCardPropertiesModel.getProperty("/kpiAnnotationPath");
            ...
            if (oCustomParameters && !oCustomParameters.bStaticLinkListIndex) {
                ...
                bIgnoreEmptyString = this._processCustomParameters(
                    oCustomParameters,
                    oSelectionVariant,
                    oContextParameters
                );
        ...
        _removeSensitiveAttributesFromNavSelectionVariant: function (oEntityType, oNavSelectionVariant) {
            for (var i = 0; i < oEntityType.property.length; i++) {
                if (
                    oEntityType.property[i]["com.sap.vocabularies.PersonalData.v1.IsPotentiallySensitive"] &&
                    oEntityType.property[i]["com.sap.vocabularies.PersonalData.v1.IsPotentiallySensitive"]
                ) {
            ...
            oPromise.then(
                ...
                function (oError) {
                    ...
                    if (errorMessage === "" && oError.actionText) {
                        errorMessage =
                            OvpResources.getText("Toast_Action_Error") + ' "' + oError.actionText + '"' + ".";
                    }
        ...
        setErrorState: function () {
            ...
            Component.create(oComponentConfig).then(function (oLoadingCard) {
                ...
                if (!!aAggregation) {
                    if (bFlag) {
                        ...
                        if (aAggregation.getItems()[0] && aAggregation.getItems()[0].getDomRef()) {
                            iHeight = jUtils.getOuterHeight(aAggregation.getItems()[0].getDomRef());
                        }
                    } else {
         ...
         */
         checkIBNNavigationExistsForCard: function() {
            ...
            if (oCardPropsModel && oEntityType && sContentFragment) {
                ...
                if (sContentFragment === "sap.ovp.cards.v4.list.List" || sContentFragment === "sap.ovp.cards.v4.table.Table") {
                    var bIsHeaderNavigable = NavigationHelper.checkIdentificationAnnotationNavigation(oCardPropsModel, oEntityType, oEntityModel),
                        bIsLineItemNavigable = NavigationHelper.checkLineItemNavigation(oCardPropsModel, oEntityType, oEntityModel);
    
                    return bIsHeaderNavigable || bIsLineItemNavigable;
                }

                if (sContentFragment === "sap.ovp.cards.v4.charts.analytical.analyticalChart") {
                    var bHeaderNavExistsForChart = NavigationHelper.checkHeaderNavForChart(oCardPropsModel),
         ...
         * @method onCardManage
         */
        onCardManage: function (oEvent) {
            this.oMainComponent._onManageCardsMenuButtonPress();
        }
    });
});
