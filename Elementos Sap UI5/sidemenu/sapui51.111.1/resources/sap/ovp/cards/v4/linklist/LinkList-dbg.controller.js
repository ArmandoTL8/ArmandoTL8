sap.ui.define([
    ...
    var oLogger = new OVPLogger("OVP.v4.linklist.LinkList");

    return CardController.extend("sap.ovp.cards.v4.linklist.LinkList", {
        onInit: function () {
            ...
            this.oListRow = this.byId("ovpListRow");
            if (this.oListRow) {
                this.oListRow.addStyleClass("ovpListRowNoData");
            }
        ...
        onBeforeRendering: function (oEvent) {
            ...
            if (
                oCardPropertiesModel.getProperty("/listFlavor") === "standard" &&
        ...
        onAfterRendering: function (oEvent) {
                ...
                case "carousel":
                    ...
                    if (oCardPropertiesModel.getProperty("/staticContent")) {
                        ...
                        if (
                            iRows &&
                            !(
                                !oCardPropertiesModel.getProperty("/defaultSpan") &&
                                oCardPropertiesModel.getProperty("/cardLayout/autoSpan")
                            )
                        ) {
            ...
            if (
                ...
                oCard.dashboardLayout.showOnlyHeader
                    ? element.classList.add("sapOvpMinHeightContainer")
                    : element.classList.remove("sapOvpMinHeightContainer");
            }
        ...
        settingHeightAndWidthOfLinkListCardsOnManageCardAndFilter: function () {
            if (this._aLinkListIds) {
                var oCardPropertiesModel = this.getCardPropertiesModel();
                //Added to fix the linklist card collapsing issue when list item is clicked and also when card is added from manage cards
                var iRows = oCardPropertiesModel.getProperty("/cardLayout/rowSpan");
                if (
                    !oCardPropertiesModel.getProperty("/defaultSpan") &&
                    oCardPropertiesModel.getProperty("/cardLayout/autoSpan")
        ...
        itemOnBeforeRendering: function (oEvent) {
            var iRows,
                oCardPropertiesModel = this.getCardPropertiesModel();
            switch (oCardPropertiesModel.getProperty("/listFlavor")) {
                case "standard":
                    //Calculate default row span if defaultSpan configuration is not given for standard linklist card in manifest
                    //Added to fix list items not getting displayed on page load issue, due to iRows sending incorrect values & also to fix extra space in linklist card issue
                    if (
                        !oCardPropertiesModel.getProperty("/defaultSpan") &&
                        !oCardPropertiesModel.getProperty("/staticContent") &&
                        oCardPropertiesModel.getProperty("/cardLayout/autoSpan")
                ...
                case "carousel":
                    oLogger.info("FYI: currently nothing special to handle here");
                    break;
                default:
                    break;
            }
        ...
        getSpansWhenNoDefaultSpanMentioned: function () {
            ...
            var iNoOfItems =
                (aStaticContent && aStaticContent.length) ||
                (this.getCardItemsBinding() && this.getCardItemsBinding().getLength()) ||
                0;
            var iLinkListItemHeight = aStaticContent
                ? this.getItemHeight(this, OVPLINKLIST, true)
                : oCardPropertiesModel.getProperty("/cardLayout/itemHeight");
            var iHeaderHeight = 0;
            ...
            return Math.ceil(
                (iLinkListItemHeight * Math.min(iNoOfItems, 6) +
                    (iHeaderHeight + LINKLIST_BORDER_TOP + LINKLIST_BORDER_BOTTOM)) /
                iRowHeight
            );
        },
            ...
            switch (oCardPropertiesModel.getProperty("/listFlavor")) {
                case "standard":
                    ...
                    try {
                        ...
                        oLogger.info("FYI: Unable to remove the delagted event at the last item of the last list");
                    }
                ...
                case "carousel":
                    ...
                    try {
                        ...
                        oLogger.info("FYI: Unable to remove the delagted event on the carousel");
                    }
                    //If defaultSpan property is not mentioned in manifest and card is loading for the first time then load as fixed card layout by ignoring iRows
                    if (
                        iRows &&
        ...
        _itemOnEventBuildStandard: function (oCardPropertiesModel, iCols, iRows, bInitiaLoad) {
            ...
            if (iCardItems !== undefined) {
                ...
                var iItemHeight = oCardPropertiesModel.getProperty("/staticContent")
                    ? this.getItemHeight(this, OVPLINKLIST, true)
                    : oCardPropertiesModel.getProperty("/cardLayout/itemHeight");
            ...
            if (this._iVisibleColums > 1) {
                ...
                for (var j = this._iNoOfItemsPerColumn; j < iPossibleItems; j++) {
                    //Loop over all items for the card
            ...
            if (this._aLinkListIds) {
                ...
                if (oBindingInfo) {
                    ...
                    if (iNewLengthTotal > this._iAvailableItems && oBindingInfo.length <= this._iAvailableItems) {
                        ...
                        this._itemOnEventBuildStandard(
                            oCardPropertiesModel,
                            newCardLayout.colSpan,
                            newCardLayout.rowSpan,
                            false
                        );
                    }
                } else {
        ...
        _setListHeightInCarouselCard: function (iRows) {
            ...
            if (iRows !== -1) {
                ...
                iCarouselHeight =
                    iRows * iRowHeight - (iHeaderHeight + CAROUSEL_PAGE_PADDING + CAROUSEL_PAGE_INDICATOR);
        ...
        _setCarouselImageProperties: function () {
            var oCarousel = this.byId(PICTURECAROUSEL),
                ...
                iNavigatorHeight =
                    aPages.length > 1
                        ? oCarousel.$().find(".sapMCrslControlsBottom.sapMCrslControls").outerHeight()
                        : 0,
                oPage = null,
                oImg = null,
                iTextHeight = 0,
            ...
            for (var iPage = 0; iPage < aPages.length; iPage++) {
                ...
                if (oPage) {
                    ...
                    if (oElement) {
                        ...
                        iTextHeight = oElement.hasStyleClass("sapOvpCarouselContentHeader")
                            ? oElement.$().outerHeight()
                            : 0;
        ...
        _setCarouselImageHeight: function (oImg, ioImgHeight) {
            ...
            if (ioImgNaturalHeight !== 0) {
                ...
                ioImgNaturalHeight < ioImgHeight
                    ? oImg.setHeight(ioImgNaturalHeight + "px")
                    : oImg.setHeight(ioImgHeight + "px");
            }
        },
        ...
        _getListHeightInStandardCard: function (iRows) {
            ...
            if (iRows) {
                ...
                var iRowHeight = oCardPropertiesModel.getProperty("/cardLayout/iRowHeightPx");
                iLinkListHeight =
                    iRows * iRowHeight - (iHeaderHeight + LINKLIST_BORDER_TOP + LINKLIST_BORDER_BOTTOM);
            }
            ...
            if (OVPCardAsAPIUtils.checkIfAPIIsUsed(this)) {
                ...
                if (sTargetUrl) {
                    ...
                    if (sInNewWindow === "true") {
                        openWindow(sTargetUrl, "newWindow");
                    } else {
        ...
        onLinkListLineItemUrl: function (oEvent) {
            if (OVPCardAsAPIUtils.checkIfAPIIsUsed(this)) {
                ...
                if (oNavigation.hasOwnProperty("String")) {
                    ...
                    var oNavigationFields = this.getEntityNavigationEntries(
                        oEvent.getSource().getBindingContext(),
                        sAnnotationPath
                    )[0];
                ...
                if (sTargetUrl) {
                    ...
                    oLogger.info("LinkList Standard Card: No Navigation on line item");
                }
            }
        },
