...
sap.ui.define([
    ...
    var aSupportedChartTypesForSemanticColors = [
        constants.COLUMNSTACKED_CHARTTYPE,
        constants.DONUT_CHARTTYPE,
        constants.COLUMN_CHARTTYPE,
        constants.BAR_CHARTTYPE,
        constants.VERTICALBULLET_CHARTTYPE,
        constants.LINE_CHARTTYPE,
        constants.COMBINATION_CHARTTYPE,
        constants.BUBBLE_CHARTTYPE
    ];

    // Supported chart type for coloring with stable dimensions
    var aSupportedChartTypesForStableColors = [constants.COLUMNSTACKED_CHARTTYPE];
    var oGlobalEntityType;

    var oLogger = new OVPLogger("OVP.charts.VizAnnotationManager");

    function isV4TimeSeriesSemantics(oDataModel, sEntitySet, dimensionName) {
        var semantics = oDataModel.getMetaModel().getObject("/" + sEntitySet + "/" + dimensionName + "@");
        return (
    ...
    function formatItems(
        ...
        if (!iContext || !iContext.getSetting("ovpCardProperties")) {
            oLogger.error(errorMessages.ANALYTICAL_CONFIG_ERROR);
            ret += "}";
            return ret;
        }
        ...
        var unitKey_v4_isoCurrency = "@" + constants.UNIT_KEY_V4_ISOCurrency; //as part of supporting V4 annotation Uom
        var unitKey_v4_unit = "@" + constants.UNIT_KEY_V4_Unit; //as part of supporting V4 annotation Uom
        var oSelectItems = {};
        ...
        if (bParams) {
            var path = CardAnnotationHelper.resolveParameterizedEntitySet(
                dataModel,
                oEntitySet,
                oSelectionVariant,
                aParameters
        ...
        if (bSorter) {
            ...
            if (oSortAnnotationCollection.length < 1) {
                oLogger.warning(errorMessages.CARD_WARNING + errorMessages.SORTORDER_WARNING);
            } else {
                var sSorterValue = "";
                var oSortOrder;
                var sSortOrder;
                ...
                for (var i = 0; i < oSortAnnotationCollection.length; i++) {
                    ...
                    if (typeof oSortOrder.Descending === "undefined") {
                        ...
                        if (!checkFlag) {
                            oLogger.warning(errorMessages.CARD_WARNING + errorMessages.BOOLEAN_ERROR);
                            sSortOrder = "true";
                        } else {
        ...
        //measurelist
        var measureAtrributeStrings = [];
        measuresList.forEach(function (measureListElement, i) {
            var sMeasureAggregateAxis = measureAggregate && measureAggregate["Axis" + (i + 1)];
            var measureAggregateAxis = sMeasureAggregateAxis ? sMeasureAggregateAxis : "sum";
            var measureAttriString = [measureListElement, " with ", measureAggregateAxis, " as ", measureListElement, "Aggregate"].join("");
            measureAtrributeStrings.push(measureAttriString);
        });
        ret += measureAtrributeStrings.join(",");
        ret += "))";
    ...
    function checkForecastMeasure(aMeasure, entityTypeObject) {
        ...
        if (boolflag === true) {
            ...
            oLogger.warning(errorMessages.CARD_WARNING + errorMessages.INVALID_FORECAST);
            return undefined;
        }
    }
    ...
    function checkTargetMeasure(aMeasure, entityTypeObject) {
        ...
        if (boolflag === true) {
            ...
            oLogger.warning(errorMessages.CARD_WARNING + errorMessages.INVALID_TARGET);
            return undefined;
        }
    }
            ...
            each(feeds, function (i, v) {
                if (v.getUid() === "valueAxis" || v.getUid() === "categoryAxis") {
                    if (v.getValues().length != 1) {
                        boolFlag = false;
                        oLogger.warning(errorMessages.CARD_WARNING + errorMessages.INVALID_DIMMEAS);
                        return false;
                    }
                }
            });
    ...
    function formatByType(oMetadata, sProp, sVal) {
        ...
        var aNumberTypes = ["Edm.Int", "Edmt.Int16", "Edm.Int32", "Edm.Int64", "Edm.Decimal"];
    ...
    /*
     *  Formatter for chart axis in vizFrame
     */
    function formatChartAxes(reset) {
        var customFormatter = {
            locale: function () { },
            format: function (value, pattern) {
                ...
                if (patternArr.length > 0) {
                    ...
                    } else if (patternArr[0] === "tooltipNoScaleFormatter") {
                        //Pattern for tooltip other than Date
                ...
                if (value.constructor === Date) {
                    ...
                    // BCP: 2080357473
                    // Use 24 hours locale format when pattern is "hh:mm", to be applied in the navigated application
                    var oMainController = CommonUtils.getApp();
                    var bResetChartAxisFormatter = oMainController && oMainController.bResetChartAxisFormatter;

                    if (bResetChartAxisFormatter && pattern === "hh:mm") {
                        var sLocaleTime = value.toLocaleTimeString();
                        var aLocaleTime = sLocaleTime.split(":");
                        value = aLocaleTime[0] + ":" + aLocaleTime[1];
                        return value;
                    }
                    var bUTC = bResetChartAxisFormatter ? false : true;
                    value = oDateFormat.format(new Date(value), bUTC);
                }
                return value;
            }
        };
        ...
        for (var i = 0; i < feeds.length; i++) {
            if (feeds[i].getUid() === feedName) {
                ...
                if (oCardsModel && oCardsModel.getProperty("/bInsightEnabled")) {
                    ...
                    AnalyticalCardHelper.setVizPropertyMap(sKey, { categoryAxis: { title: { visible: false } } });
                }
                return;
            }
        }
    ...
    function checkExists(term, annotation, type, bMandatory, logViewId, contentFragment) {
        ...
        var annoTerm;
        if (!term && bMandatory) {
            oLogger.error(logViewId + errorMessages.CARD_ERROR + type + errorMessages.IS_MANDATORY);
            return ret;
        }
        if (!term) {
            /* Optional parameters can be blank */
            oLogger.warning(logViewId + errorMessages.CARD_WARNING + type + errorMessages.IS_MISSING);
            ret = true;
            return ret;
        }
        ...
        if (
            contentFragment &&
            contentFragment === "sap.ovp.cards.v4.charts.analytical.analyticalChart" &&
            type === "Chart Annotation" &&
            (!annoTerm.ChartType || !annoTerm.ChartType.$EnumMember)
        ) {
            oLogger.error(
                logViewId + errorMessages.CARD_ERROR + errorMessages.MISSING_CHARTTYPE + errorMessages.CHART_ANNO
            );
            return ret;
        }
        ret = true;
    ...
    function validateCardConfiguration(oController) {
        ...
        var oEntitySetAnnotations = oMetaModel.getData().$Annotations[entityTypeData.$Type];
        if (!entityTypeData || isEmptyObject(entityTypeData)) {
            oLogger.error(logViewId + errorMessages.CARD_ERROR + "in " + errorMessages.CARD_ANNO);
            return ret;
        }
        ...
        ret = checkExists(selVar, oEntitySetAnnotations, "Selection Variant", false, logViewId);
        ret = checkExists(chartAnno, oEntitySetAnnotations, "Chart Annotation", true, logViewId, contentFragment) && ret;
        ret = checkExists(preVar, oEntitySetAnnotations, "Presentation Variant", false, logViewId) && ret;
        ret = checkExists(idAnno, oEntitySetAnnotations, "Identification Annotation", true, logViewId) && ret;
        ret = checkExists(dPAnno, oEntitySetAnnotations, "Data Point", false, logViewId) && ret;
        return ret;
    }
    ...
    function checkNoData(oEvent, cardContainer, vizFrame) {
        /*
            ...
            if (!cardContainer) {
                oLogger.error(
                    errorMessages.CARD_ERROR + 
                    errorMessages.CARD_CONTAINER_ERROR +
                    "(" + vizFrame.getId() + 
                    ")"
                );
                return;
            ...
            if (
                ...
                oLogger.error(
                    errorMessages.CARD_ERROR + 
                    errorMessages.DATA_UNAVAIALABLE  +
                    "(" + vizFrame.getId() + 
                    ")"
                );
    ...
    function hasTimeSemantics(aDimensions, config, dataModel, entitySet) {
        ...
        if (!config.time || isEmptyObject(config.time)) {
            return ret;
        }
        if (!aDimensions) {
            return ret;
        ...
        for (var index = 0; index < aDimensions.length; index++) {
            ...
            if (oMetadata && oMetadata[dimensionName]) {
                ...
                sapSemanticsV4 = isV4TimeSeriesSemantics(dataModel, entitySet, dimensionName); //as part of supporting V4 annotation
            }
    ...
    function checkRolesForProperty(queue, config, type) {
        ...
        each(queue, function (i, val) {
            if (!val || !val[feedtype] || !val[feedtype].PropertyPath) {
                oLogger.error(errorMessages.INVALID_CHART_ANNO);
                return false;
            }
        ...
        oLogger.error(
            errorMessages.CARD_ERROR +
            errorMessages.INVALID_REDUNDANT +
            type +
            "(s) " +
            queuedNames.join(", ") +
            ". " +
            errorMessages.ALLOWED_ROLES +
            config.type +
            errorMessages.CHART_IS +
            allowedRoles
        );
    }
    ...
    function formThePathForCriticalityStateCalculation(iContext, oDataPoint) {
        ...
        if (
            ...
            return CardAnnotationHelper._calculateCriticalityState(
                ...
                toleranceLow,
                toleranceHigh,
                CardAnnotationHelper.criticalityConstants.StateValues
            );
        } else {
                ...
                if (
                    ...
                    if (endsWith(sImproveDirection, "Minimize") || endsWith(sImproveDirection, "Minimizing")) {
                        if (toleranceHigh && deviationHigh) {
                            ...
                            oLogger.warning(errorMessages.CARD_WARNING + errorMessages.INVALID_CRITICALITY);
                        }
                    } else if (
                    ...
                    oLogger.warning(errorMessages.CARD_WARNING + errorMessages.INVALID_IMPROVEMENT_DIR);
                }
            }
        });
        if (boolFlag === true && aMeasures.length > 1) {
            oLogger.warning(errorMessages.CARD_WARNING + errorMessages.INVALID_SEMANTIC_MEASURES);
        }
        return boolFlag;
    }
    ...
    function buildSemanticLegends(oMeasure, entityTypeObject, oMetadata, vizFrame) {
        ...
        if (oMetadata[measureName]) {
            if (oMetadata[measureName][constants.LABEL_KEY_V4]) {
                //as part of supporting V4 annotation
                measureName = getLabelFromAnnotationPath(
                    oMetadata[measureName][constants.LABEL_KEY_V4].String
                        ? oMetadata[measureName][constants.LABEL_KEY_V4].String
                        : oMetadata[measureName][constants.LABEL_KEY_V4].Path,
                    vizFrame,
                    oMetadata
                );
    ...
    function checkNumberFormat(minValue, val, entityTypeObject) {
        if (val && val.DataPoint && val.DataPoint.AnnotationPath) {
            ...
            if (
                fractionDigits &&
                fractionDigits.NumberOfFractionalDigits &&
                fractionDigits.NumberOfFractionalDigits.Int
            ) {
    ...
    function isMeasureCurrency(oMetadata, sapUnit) {
        ...
        if (
            ...
            (oMetadata[sapUnit]["Org.OData.Measures.V1.ISOCurrency"] ||
                (oMetadata[sapUnit][constants.SEMANTICS_KEY] &&
                    oMetadata[sapUnit][constants.SEMANTICS_KEY] === constants.CURRENCY_CODE))
        ) {
            return true;
        }
        return false;
    ...
    function checkEDMINT32Exists(oMetadata, val, feedtype) {
        if (oMetadata[val[feedtype].$PropertyPath].$Type === constants.EDM_INT32) {
            return true;
        }
        return false;
    }
    ...
    function checkEDMINT64Exists(oMetadata, val, feedtype) {
        if (oMetadata[val[feedtype].$PropertyPath].$Type === constants.EDM_INT64) {
            return true;
        }
        return false;
    }
    ...
    function buildVizAttributes(vizFrame, handler, oController) {
        ...
        if (!config) {
            oLogger.error(
                ...
                "in " +
                errorMessages.CARD_CONFIG +
                errorMessages.CARD_CONFIG_ERROR +
                chartType +
                " " +
                errorMessages.CARD_CONFIG_JSON
            );
            return;
        }
        ...
        if (!oCardsModel) {
            oLogger.error(
                errorMessages.CARD_ERROR + "in " + errorMessages.CARD_CONFIG + errorMessages.NO_CARD_MODEL
            );
            return;
        }
        var dataModel = vizFrame.getModel();
        ...
        if (!entityTypeObject) {
            oLogger.error(errorMessages.CARD_ANNO_ERROR + "in " + errorMessages.CARD_ANNO);
            return;
        }
        var oMetadata = getMetadata(dataModel, entitySet);
        //add annotations to inline metadata if not present
        ...
        var sChartTitle = "";
        if (entityTypeObject && entityTypeObject['@' + sChartAnnotationPath] && entityTypeObject['@' + sChartAnnotationPath].Title && entityTypeObject['@' + sChartAnnotationPath].Title.String) {
            sChartTitle = entityTypeObject['@' + sChartAnnotationPath].Title.String;
            if (sChartTitle.match(/{[@]?i18n>.+}/gi) || sChartTitle.match(/{[@]?i18n&gt;.+}/gi)) {
                var oResourceBundle = vizFrame.getModel("@i18n").getResourceBundle();
                var indexOfGt = sChartTitle.indexOf(">") > -1 ? sChartTitle.indexOf(">") : sChartTitle.indexOf("&gt;") + 3;
        ...
        if (!(aDimensions = chartContext.DimensionAttributes) || !aDimensions.length) {
            oLogger.error(
                errorMessages.CHART_ANNO_ERROR +
                "in " +
        ...
        if (
            ...
            if (colorPaletteDimension) {
                if (oMetadata && oMetadata[colorPaletteDimension]) {
                    if (oMetadata[colorPaletteDimension][constants.LABEL_KEY_V4]) {
                        //as part of supporting V4 annotation
                        feedName = getLabelFromAnnotationPath(
                            oMetadata[colorPaletteDimension][constants.LABEL_KEY_V4]
                                ? oMetadata[colorPaletteDimension][constants.LABEL_KEY_V4]
                                : oMetadata[colorPaletteDimension][constants.LABEL_KEY_V4].Path,
                            vizFrame,
                            oMetadata
                        );
                ...
                if (oCardsModel && oCardsModel.getProperty("/bInsightEnabled")) {
                    ...
                    var sColorPaletteDimensionType = oMetadata && oMetadata[colorPaletteDimension] && oMetadata[colorPaletteDimension][constants.TYPE_KEY] || "";
        ...
        var bIsMinMax = false;
        if (
            chartContext.ChartType.$EnumMember === constants.SCATTER_CHARTTYPE ||
                ...
                if (
                    ...
                    if (entityTypeObject && entityTypeObject[sDataPointPath]) {
                        ...
                        if (
                            ...
                            oLogger.error(errorMessages.ERROR_MISSING_AXISSCALES);
                        }
                    }
                }
        ...
        */
            ...
            if (feed.type) {
                ...
                if (!feed.role) {
                    ...
                    for (var j = 0; j < len && aFeedProperties.length < max; ++j) {
                        ...
                        if (oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY_V4_ISOCurrency]) {
                            //as part of supporting V4 annotation
                            sapUnit = oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY_V4_ISOCurrency]
                                .$Path
                                ? oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY_V4_ISOCurrency].$Path
                                : undefined;
                        } else if (oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY_V4_Unit]) {
                            sapUnit = oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY_V4_Unit].$Path
                                ? oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY_V4_Unit].$Path
                                : undefined;
                        ...
                        for (var k = 0; k < len && aFeedProperties.length < max; ++k) {
                            ...
                            if (
                                ...
                                if (annotationRole === role) {
                                    ...
                                    if (oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY_V4_ISOCurrency]) {
                                        //as part of supporting V4 annotation
                                        sapUnit = oMetadata[val[feedtype].$PropertyPath][
                                            constants.UNIT_KEY_V4_ISOCurrency
                                        ].$Path
                                            ? oMetadata[val[feedtype].$PropertyPath][
                                                constants.UNIT_KEY_V4_ISOCurrency
                                            ].$Path
                                            : undefined;
                                    } else if (oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY_V4_Unit]) {
                                        sapUnit = oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY_V4_Unit]
                                            .$Path
                                            ? oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY_V4_Unit]
                                                .$Path
                                            : undefined;
                                    } else if (oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY]) {
                                        sapUnit = oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY];
                                    ...
                                    if (!isCurrency) {
                                        ...
                                        minValCurr = checkNumberFormat(minValCurr, val, entityTypeObject);
                                        maxScaleValueCurr = getMaxScaleFactor(
                                            maxScaleValueCurr,
                                            val,
                                            entityTypeObject
                                        );
                            ...
                            if (
                                ...
                                if (oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY_V4_ISOCurrency]) {
                                    ...
                                    sapUnit = oMetadata[val[feedtype].$PropertyPath][
                                        ...
                                        ? oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY_V4_ISOCurrency]
                                            .$Path
                                        : undefined;
                                } else if (oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY_V4_Unit]) {
                                    sapUnit = oMetadata[val[feedtype].$PropertyPath][constants.UNIT_KEY_V4_Unit]
                    ...
                    if (aFeedProperties.length < min) {
                        oLogger.error(
                            errorMessages.CARD_ERROR +
                            errorMessages.MIN_FEEDS +
                            chartType +
                            " " +
                            errorMessages.FEEDS_OBTAINED +
                            aFeedProperties.length +
                            " " +
                            errorMessages.FEEDS_REQUIRED +
                            min +
                            " " +
                            errorMessages.FEEDS
                        );
                ...
                if (aFeedProperties.length) {
                    ...
                    if (!(dataset = vizFrame.getDataset())) {
                        oLogger.error(errorMessages.NO_DATASET);
                        return false;
                    }
                    ...
                    each(aFeedProperties, function (i, val) {
                        ...
                        if (oMetadata && oMetadata[property]) {
                            if (oMetadata[property]["annotations"]) {
                                if (oMetadata[property]["annotations"]["@" + constants.LABEL_KEY_V4]) {
                                    //as part of supporting V4 annotation
                                    if (
                                        oMetadata[property]["annotations"]["@" + constants.LABEL_KEY_V4] instanceof
                                        String &&
                                        oMetadata[property]["annotations"]["@" + constants.LABEL_KEY_V4]
                                    ) {
                                        feedName = getLabelFromAnnotationPath(
                                            oMetadata[property]["annotations"]["@" + constants.LABEL_KEY_V4],
                                            vizFrame,
                                        ...
                                        oMetadata[property]["annotations"]["@" + constants.LABEL_KEY_V4].$Path
                                    ) {
                                        feedName =
                                            oMetadata[property]["annotations"]["@" + constants.LABEL_KEY_V4].$Path;
                                    } else {
                                        feedName = oMetadata[property]["annotations"]["@" + constants.LABEL_KEY_V4]; //Fallback on the technical name in case sap:label is not maintained in the desired language
                                    }
                                } else if (oMetadata[property]["annotations"]["@" + constants.LABEL_KEY]) {
                                    feedName = oMetadata[property]["annotations"]["@" + constants.LABEL_KEY];
                                } else if (property) {
                                    feedName = property;
                                }
                                if (oMetadata[property]["annotations"]["@" + constants.TEXT_KEY_V4]) {
                                    //as part of supporting V4 annotation
                                    textColumn = oMetadata[property]["annotations"]["@" + constants.TEXT_KEY_V4].$String
                                        ? oMetadata[property]["annotations"]["@" + constants.TEXT_KEY_V4].$String
                                        : oMetadata[property]["annotations"]["@" + constants.TEXT_KEY_V4].$Path;
                                } else if (oMetadata[property]["annotations"]["@" + constants.TEXT_KEY]) {
                                    textColumn = oMetadata[property]["annotations"]["@" + constants.TEXT_KEY];
                                }
                            } else if (property) {
                                textColumn = property;
                            }
                        ...
                        if (
                            ...
                            displayBindingPath =
                                "{path:'" +
                                property +
                                "', formatter: 'sap.ovp.cards.v4.charts.VizAnnotationManager.returnDateFormat'}";
                        } else {
                            //Check if there's a text arrangement annotation
                            if (
                                oMetadata[property] &&
                                oMetadata[property]["annotations"] &&
                                oMetadata[property]["annotations"]["@" + constants.TEXT_KEY_V4] &&
                                oMetadata[property]["annotations"][
                                "@" + constants.TEXT_KEY_V4 + "@" + constants.TEXT_ARRANGEMENT_ANNO
                                ]
                            ) {
                                ...
                                if (!sTextArrangementType) {
                                    oTextArrangement =
                                        oMetadata[property]["annotations"]["@" + constants.TEXT_KEY_V4][
                                        "@" + constants.TEXT_ARRANGEMENT_ANNO
                                        ];
                                    sTextArrangementType = oTextArrangement && oTextArrangement.$EnumMember;
                                }
                                ...
                                if (!sTextArrangementType) {
                                    ...
                                    oTextArrangement =
                                        oEntityType && oEntityType["@" + constants.TEXT_ARRANGEMENT_ANNO];
                                    sTextArrangementType = oTextArrangement && oTextArrangement.$EnumMember;
                                }
                        ...
                        if (feedtype === "Dimension") {
                            ...
                            var isV4CalendarYear = isV4TimeSeriesSemantics(
                                vizFrame.getModel(),
                                entitySet,
                                property
                            ); //as part of supporting V4 annotation
            ...
            each(feeds, function (i, v) {
                if (
                    ...
                    if (selectedValue) {
                        ...
                        var forecastMeasure =
                            entityTypeObject[selectedValue.DataPoint.AnnotationPath.substring(1)].ForecastValue
                                .PropertyPath ||
                            entityTypeObject[selectedValue.DataPoint.AnnotationPath.substring(1)].ForecastValue
                                .Path;
        ...
        if (
            bEnableStableColors &&
            aSupportedChartTypesForStableColors.indexOf(chartContext.ChartType.$EnumMember) >= 0 &&
        ...
        each(feeds, function (i, feed) {
            ...
            if (feedType === "Measure") {
                ...
                var aFeedValueContexts = chartContext.MeasureAttributes.filter(function (measureAttr) {
                    var sPropertyPath = measureAttr[feedType].$PropertyPath;
                                ...
                                if (measureObject["Measure"] && measureObject["Measure"]["PropertyPath"]) {
                                    ...
                                    if (
                                        (oMetadata[measureProperty] &&
                                            oMetadata[measureProperty][constants.LABEL_KEY] ===
                                            trimmedPropertyLabel) ||
                                        (oMetadata[measureProperty] &&
                                            oMetadata[measureProperty][constants.LABEL_KEY_V4] ===
                                            trimmedPropertyLabel)
                                    ) {
                                        return true;
                                    } else {
    ...
    function getFeednameByProperty(oMetadata, vizFrame, property) {
        var feedName = "";
        if (oMetadata && oMetadata[property]) {
            if (oMetadata[property].annotations && oMetadata[property].annotations["@" + constants.LABEL_KEY_V4]) {
                //as part of supporting V4 annotation
                feedName = getLabelFromAnnotationPath(
                    oMetadata[property].annotations["@" + constants.LABEL_KEY_V4],
                    vizFrame,
                            ...
                            if (dimensionArrayNames[j] === dimensions[k]) {
                                for (var key in oMetadata) {
                                    if (oMetadata.hasOwnProperty(key)) {
                                        var propertyName =
                                            oMetadata[key][constants.LABEL_KEY] ||
                                            oMetadata[key][constants.NAME_KEY] ||
                                            oMetadata[key][constants.NAME_CAP_KEY];
                            ...
                            if (dimensionArrayNames[j] === dimensions[k]) {
                                for (var key in oMetadata) {
                                    if (annotations[oEntityType.$Type + "/" + key]) {
                                        var propertyName =
                                            (typeof annotations[oEntityType.$Type + "/" + key]["@" + constants.LABEL_KEY_V4] === "string" &&
                                                annotations[oEntityType.$Type + "/" + key]["@" + constants.LABEL_KEY_V4]) ||
                                            annotations[oEntityType.$Type + "/" + key][constants.LABEL_KEY] ||
                                            annotations[oEntityType.$Type + "/" + key][constants.NAME_KEY] ||
                                            annotations[oEntityType.$Type + "/" + key][constants.NAME_CAP_KEY] ||
                                            key;
    ...
    function getMaxItems(vizFrame) {
        ...
        if (typeof maxItemTerm === "number" || (typeof maxItemTerm === "string" && !isNaN(maxItemTerm))) {
            var maxItems = parseInt(maxItemTerm, 10);
            return {
