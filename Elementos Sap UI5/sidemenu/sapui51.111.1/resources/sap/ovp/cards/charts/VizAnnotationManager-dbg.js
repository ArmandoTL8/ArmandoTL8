...
sap.ui.define([
    ...
    function checkNoData(oEvent, cardContainer, vizFrame) {
        /*	var self = sap.ovp.cards.charts.VizAnnotationManager;
         var data, noDataDiv;
         if (!cardContainer) {
         oLogger.error(errorMessages.CARD_ERROR + errorMessages.CARD_CONTAINER_ERROR +
         "(" + vizFrame.getId() + ")");
         return;
         }
         ...
         oLogger.error(errorMessages.CARD_ERROR + errorMessages.DATA_UNAVAIALABLE  +
         "(" + vizFrame.getId() + ")");
    ...
    function hasTimeSemantics(aDimensions, config, dataModel, entitySet) {
        ...
        for (var index = 0; index < aDimensions.length; index++) {
            ...
            if (oMetadata && oMetadata[dimensionName]) {
                ...
                sapSemanticsV4 =
                    oMetadata[dimensionName]["com.sap.vocabularies.Common.v1.IsCalendarYear"] ||
                    oMetadata[dimensionName]["com.sap.vocabularies.Common.v1.IsCalendarYearMonth"] ||
                    oMetadata[dimensionName]["com.sap.vocabularies.Common.v1.IsCalendarYearQuarter"] ||
                    oMetadata[dimensionName]["com.sap.vocabularies.Common.v1.IsCalendarYearWeek"]; //as part of supporting V4 annotation
            }
    ...
    function buildVizAttributes(vizFrame, handler, oController) {
        ...
        [config.dimensions, config.measures].forEach(function (entry, i) {
            ...
            if (entry.min && oProperty.length < entry.min) {
                oLogger.error(errorMessages.CARD_ERROR + "in " + chartType + " " + errorMessages.CARD_LEAST + entry.min + " " + typeCue);
                bErrors = true;
            }
            if (entry.max && oProperty.length > entry.max) {
                oLogger.error(errorMessages.CARD_ERROR + "in " + chartType + errorMessages.CARD_MOST + entry.max + " " + typeCue);
                bErrors = true;
            }
        });
                ...
                if (oMetadata && oMetadata[colorPaletteDimension]) {
                    if (oMetadata[colorPaletteDimension][constants.LABEL_KEY_V4]) {
                        ...
                        feedName = getLabelFromAnnotationPath(
                            oMetadata[colorPaletteDimension][constants.LABEL_KEY_V4].String
                                ? oMetadata[colorPaletteDimension][constants.LABEL_KEY_V4].String
        ...
        */
                ...
                if (!feed.role) {
                    ...
                    for (var j = 0; j < len && aFeedProperties.length < max; ++j) {
                        ...
                        if (oMetadata[val[feedtype].PropertyPath][constants.UNIT_KEY_V4_ISOCurrency]) {
                            //as part of supporting V4 annotation
                            sapUnit = oMetadata[val[feedtype].PropertyPath][constants.UNIT_KEY_V4_ISOCurrency].Path
                ...
                if (aFeedProperties.length) {
                    ...
                    each(aFeedProperties, function (i, val) {
                        ...
                        if (oMetadata && oMetadata[property]) {
                            if (oMetadata[property][constants.LABEL_KEY_V4]) {
                                //as part of supporting V4 annotation
                                if (oMetadata[property][constants.LABEL_KEY_V4].String) {
                                    feedName = getLabelFromAnnotationPath(
                                        oMetadata[property][constants.LABEL_KEY_V4].String,
                                        vizFrame,
                                        oMetadata
                                    );
                            ...
                            if (oMetadata[property][constants.TEXT_KEY_V4]) {
                                //as part of supporting V4 annotation
                                textColumn = oMetadata[property][constants.TEXT_KEY_V4].String
                                    ? oMetadata[property][constants.TEXT_KEY_V4].String
                                    : oMetadata[property][constants.TEXT_KEY_V4].Path;
                        ...
                        if (
                            ...
                            if (
                                oMetadata[property] &&
                                oMetadata[property][constants.TEXT_KEY_V4] &&
                                oMetadata[property][constants.TEXT_KEY_V4][constants.TEXT_ARRANGEMENT_ANNO]
                            ) {
        ...
        each(feeds, function (i, feed) {
            ...
            if (feedType === "Measure") {
                ...
                var aFeedValueContexts = chartContext.MeasureAttributes.filter(function (measureAttr) {
                    return (
                        aFeedValues.indexOf(oMetadata[measureAttr[feedType]["PropertyPath"]][constants.LABEL_KEY]) >= 0 ||
                        ...
                        each(aPropertiesLabels, function (i, propertyLabel) {
                            ...
                            var currentMeasure = aMeasures.filter(function (measureObject) {
                                if (measureObject["Measure"] && measureObject["Measure"]["PropertyPath"]) {
                                    ...
                                    if (
                                        (oMetadata[measureProperty] && oMetadata[measureProperty][constants.LABEL_KEY] === trimmedPropertyLabel) ||
        ...
        if (bSupportsTimeSemantics) {
            ...
            } else if (oMetadata[timeDimension][constants.SEMANTICS_KEY] === "year" || oMetadata[timeDimension]["com.sap.vocabularies.Common.v1.IsCalendarYear"]) {
        ...
        if (oMetadata && oMetadata[property]) {
            if (oMetadata[property][constants.LABEL_KEY_V4]) {
                ...
                feedName = getLabelFromAnnotationPath(
                    oMetadata[property][constants.LABEL_KEY_V4].String
                        ? oMetadata[property][constants.LABEL_KEY_V4].String
                        : oMetadata[property][constants.LABEL_KEY_V4].Path,
                    vizFrame,
                    oMetadata
                );
            } else if (oMetadata[property][constants.LABEL_KEY]) {
                    ...
                    for (var j = 0; j < dimensionArrayNames.length; j++) {
                        for (var k = 0; k < dimensions.length; k++) {
                            if (dimensionArrayNames[j] === dimensions[k]) {
                                for (var key in oMetadata) {
                                    if (oMetadata.hasOwnProperty(key)) {
                                        var propertyName = (oMetadata[key][constants.LABEL_KEY_V4] && oMetadata[key][constants.LABEL_KEY_V4].String) ||
                                            oMetadata[key][constants.LABEL_KEY] ||
                                            oMetadata[key][constants.NAME_KEY] ||
                                            oMetadata[key][constants.NAME_CAP_KEY];
    ...
    function dimensionAttrCheck(dimensions) {
        ...
        if (
            ...
            oLogger.error(errorMessages.CHART_ANNO_ERROR + "in " + errorMessages.CHART_ANNO + " " + errorMessages.DIMENSIONS_MANDATORY);
            return ret;
        }
        ret = true;
        return ret;
    ...
    function measureAttrCheck(measures) {
        ...
        if (
            ...
            oLogger.error(errorMessages.CHART_ANNO_ERROR + "in " + errorMessages.CHART_ANNO + " " + errorMessages.MEASURES_MANDATORY);
            return ret;
        }
        ret = true;
        return ret;
