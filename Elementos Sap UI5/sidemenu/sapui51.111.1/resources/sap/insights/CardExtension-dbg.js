...
sap.ui.define(
    ...
    function (Extension, NumberFormat, SelectionVariant, Log, DateFormat, ValueState, ValueColor, IntegrationUtils, DynamicDateUtil, Core, NavError, UrlGenerateHelper) {
        ...
        // path is kept under 3rd argument to attach watch to trigger this function for every data change
            var aContextData = this.getCard().getModel().getProperty('/content/d/results') // with batch call
        ...
        function getNavigationContext(sNavData, oContext) {
            ...
            aMandatoryParams.forEach(function(sParamName) {
                ...
                if (bJSONData && ((oSensitiveProps && !oSensitiveProps[sParamName]) || (aSensitiveProps && !aSensitiveProps.includes(sParamName)))) {
            ...
            oCardParams._relevantODataParameters.value.forEach(function(sParamName) {
                // currently doesn't support adding semantic date fields to ibnParams
                if (oCardParams[sParamName].value && !aSemanticDateFields.includes(sParamName)) {
                    if (oIbnParams) {
            ...
            aFilters.forEach(function(sFilterName) {
                var bJSONData = isJSONData(oCardParams[sFilterName].value);
                if (bJSONData && ((oSensitiveProps && !oSensitiveProps[sFilterName]) || (aSensitiveProps && !aSensitiveProps.includes(sFilterName)))) {
