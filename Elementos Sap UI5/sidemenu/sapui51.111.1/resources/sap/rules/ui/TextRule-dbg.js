...
sap.ui.define(["sap/base/Log","sap/ui/thirdparty/jquery",
		...
		ExpressionAdvanced, Link, FlexBox, Dialog, TextRuleSettings, AstYamlConverter, Constants, AstExpressionBasic, ExpressionLanguage, AstExpressionLanguage, TextRuleRenderer) {
		"use strict";
		...
		var TextRule = RuleBase.extend("sap.rules.ui.TextRule", {
			...
			_addConditionSuccess: function (newCondition, that, data) {
				...
				oDataModel.read(sPath, {
					...
					success: function (response) {
						...
						sap.m.MessageToast.show(that.oBundle.getText("branchCreatedSuccess"));
					},
					error: function () {
			...
			_deleteConditionBlock: function (oSource) {
				...
				var _deleteSuccess = function () {
					...
					sap.m.MessageToast.show(that.oBundle.getText("branchDeletedSuccess"));
				};
			...
			_getElseButton: function () {
				var that = this;
				if (!sap.ui.getCore().getElementById("_elseButton")) {
			...
			_getElseIfButton: function () {
				var that = this;
				if (!sap.ui.getCore().getElementById("_elseIfButton")) {
