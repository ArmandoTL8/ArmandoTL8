	...
	return BaseController.extend("sap.apf.ui.reuse.controller.toolbar", {
		...
		*@param {object} reset callback for save
		...
		getSaveDialog : function(bSaveAs, reset, aPath) {
			...
			var savedAPNameExist = this.oUiApi.getLayoutView().getController().oSavedPathName.getText();
			...
			function _setStateInSaveDialog(sValueState, bIsValueStateMessage, isEnabled, sValueStateMessage){
				...
				if(isEnabled === true){
					self.saveDialog.getBeginButton().setType("Emphasized");
				}
			...
			this.oInput.attachLiveChange(function(data) {
				...
				if (sSavePathName.trim() === "") {
					_setStateInSaveDialog(sap.ui.core.ValueState.Error, true, false);
				} else {
			...
			if (!bUpdatePath) {
				self.oSerializationMediator.savePath(self.analysisPathName, function(respObj, metaData, msgObj) {
					if (msgObj === undefined && (typeof respObj === "object")) {
						...
						self.oUiApi.getLayoutView().getController().setPathTitle();
				...
				var pathName;
				if (this.oCoreApi.isDirty() && this.oCoreApi.getSteps().length !== 0) {
					pathName = self.oUiApi.getLayoutView().getController().oSavedPathName.getText().slice(1, self.oUiApi.getLayoutView().getController().oSavedPathName.getText().length);
				} else {
					pathName = self.oUiApi.getLayoutView().getController().oSavedPathName.getText();
				}
