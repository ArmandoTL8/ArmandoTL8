...
sap.ui.define(
[
"sap/sac/df/firefly/ff4000.protocol.ina","sap/sac/df/firefly/ff4200.olap.api","sap/sac/df/firefly/ff4205.olap.api.base"
],
function(oFF)
{
"use strict";
...
oFF.RsDefStructureMemberProperties.prototype.getAllMemberNames = function()
{
	...
	var aliasNames = this.m_placeholderAliasMappings.getValuesAsReadOnlyListOfString();
	var iterator = aliasNames.getIterator();
...
{
	var properties = new oFF.RsLbhToUdhConversionProperties();
...
{
	var properties = new oFF.RsLbhToUdhConversionProperties();
	var universalDisplayHierarchies = dimension.getQueryModel().getUniversalDisplayHierarchies();
...
{
	var documentId = new oFF.RsDocumentId();
...
oFF.RsDocumentId.prototype.setupDimensionMembers = function(tuple)
{
	if (oFF.notNull(tuple))
	{
		...
		{
			var dimensionMember = tuple.get(i).getDimensionMember();
...
{
	var collection = this.getDocumentIdCollection();
	var rowsAxis = collection.getRowsAxis();
	var queryModel = rowsAxis.getQueryModel();
	var dataCells = queryModel.getQueryDataCells();
	...
	{
		var documentIdAction = oFF.RsDocumentIdAction.create(this, dataCell);
...
oFF.RsDocumentIdAction.create = function(documentId, qDataCell)
{
	var documentsIdsAction = new oFF.RsDocumentIdAction();
	documentsIdsAction.setupExt(documentId, qDataCell);
	return documentsIdsAction;
};
oFF.RsDocumentIdAction.prototype.m_queryDataCell = null;
oFF.RsDocumentIdAction.prototype.m_documentId = null;
oFF.RsDocumentIdAction.prototype.m_action = null;
oFF.RsDocumentIdAction.prototype.setupExt = function(documentId, qDataCell)
{
...
oFF.RsDocumentIdAction.prototype.getQueryDataCell = function()
{
...
{
	var documentIdCollection = new oFF.RsDocumentIdCollection();
...
oFF.RsDocumentIdCollection.prototype.createDocumentId = function(columnsTuple, rowsTuple)
{
	...
	var documentId = oFF.RsDocumentId.create(this, oFF.RsDocumentIdCollection.m_documentActionIdCounter, columnsTuple, rowsTuple);
...
oFF.RsDocumentIdCollection.prototype.getValidDocumentIds = function()
{
	...
	var validDocumentIds = oFF.XList.create();
	for (var k = 0; k < this.m_documentIds.size(); k++)
	{
		var validDocumentId = this.m_documentIds.get(k);
...
oFF.RsDocumentIdCollection.prototype.retainAll = function(collection)
{
	...
	for (var k = 0; k < this.m_documentIds.size(); k++)
	{
		var documentId = this.m_documentIds.get(k);
...
{
	var documentIdManager = new oFF.RsDocumentIdManager();
...
oFF.LovProcessValueHelp.serializeValueHelpForPersistedInA = function(lovProcess)
{
	var lovValueHelp = new oFF.LovProcessValueHelp();
	lovValueHelp.m_lovProcess = lovProcess;
	var dimension = lovProcess.getConfig().getDimension();
	var replacedFilters = lovValueHelp.preConfigure(lovProcess.getQueryManager());
	var serializeForValueHelp = dimension.serializeValueHelpForPersistedInA();
...
oFF.LovProcessValueHelp.prototype.execute = function(syncType, queryManager)
{
	...
	var replacedFilters = this.preConfigure(queryManager);
...
oFF.RsDataEntryCollection.prototype.getChangedDataEntries = function()
{
	...
	var orders = oFF.XCollectionUtils.sortListAsIntegers(entries.getKeysAsReadOnlyListOfString(), oFF.XSortDirection.ASCENDING);
...
oFF.RsVisualizationResultCollection = function() {};
oFF.RsVisualizationResultCollection.prototype = new oFF.XObject();
oFF.RsVisualizationResultCollection.prototype._ff_c = "RsVisualizationResultCollection";

oFF.RsVisualizationResultCollection.createInternal = function(baseResultSet, newRowAxisPartitions, newColumnAxisPartitions, newRowAxisIndices, newColumnAxisIndices)
{
	var instance = new oFF.RsVisualizationResultCollection();
	instance.m_rowIntersectionIndices = newRowAxisIndices;
	instance.m_columnIntersectionIndices = newColumnAxisIndices;
	instance.m_rowAxisPartitions = newRowAxisPartitions;
	instance.m_columnAxisPartitions = newColumnAxisPartitions;
	instance.m_baseResultSet = baseResultSet;
	return instance;
};
oFF.RsVisualizationResultCollection.prototype.m_rowAxisPartitions = null;
oFF.RsVisualizationResultCollection.prototype.m_columnAxisPartitions = null;
oFF.RsVisualizationResultCollection.prototype.m_rowIntersectionIndices = null;
oFF.RsVisualizationResultCollection.prototype.m_columnIntersectionIndices = null;
oFF.RsVisualizationResultCollection.prototype.setBaseResultSet = function(baseResultSet)
{
	this.m_baseResultSet = baseResultSet;
};
oFF.RsVisualizationResultCollection.prototype.m_baseResultSet = null;
oFF.RsVisualizationResultCollection.prototype.getForSubTupleAt = function(axisPartition, index)
{
	var newRowAxisPartitions = this.m_rowAxisPartitions;
	var newColumnAxisPartitions = this.m_columnAxisPartitions;
	var newRowIntersectionIndices = this.m_rowIntersectionIndices;
	var newColumnIntersectionIndices = this.m_columnIntersectionIndices;
	var axisPartitionBase = axisPartition;
	var subInterSectionSet = axisPartitionBase.getIntersectionSet(index);
	if (axisPartition.getAxisType() === oFF.AxisType.COLUMNS)
	{
		newColumnAxisPartitions = oFF.XStream.of(this.m_columnAxisPartitions).filter( function(cp){
			return cp !== axisPartition;
		}.bind(this)).collect(oFF.XStreamCollector.toList());
		if (oFF.isNull(this.m_columnIntersectionIndices))
		{
			newColumnIntersectionIndices = axisPartitionBase.getIntersectionSet(index);
		}
		else if (oFF.notNull(subInterSectionSet))
		{
			newColumnIntersectionIndices = oFF.XStream.of(this.m_columnIntersectionIndices).filter( function(ci){
				return subInterSectionSet.contains(ci);
			}.bind(this)).collect(oFF.XStreamCollector.toList());
		}
	}
	else
	{
		newRowAxisPartitions = oFF.XStream.of(this.m_rowAxisPartitions).filter( function(rp){
			return rp !== axisPartition;
		}.bind(this)).collect(oFF.XStreamCollector.toList());
		if (oFF.isNull(this.m_rowIntersectionIndices))
		{
			newRowIntersectionIndices = subInterSectionSet;
		}
		else if (oFF.notNull(subInterSectionSet))
		{
			newRowIntersectionIndices = oFF.XStream.of(this.m_rowIntersectionIndices).filter( function(ri){
				return subInterSectionSet.contains(ri);
			}.bind(this)).collect(oFF.XStreamCollector.toList());
		}
	}
	return oFF.RsVisualizationResultCollection.createInternal(this.m_baseResultSet, newRowAxisPartitions, newColumnAxisPartitions, newRowIntersectionIndices, newColumnIntersectionIndices);
};

oFF.RsVisualizationTupleCollection = function() {};
oFF.RsVisualizationTupleCollection.prototype = new oFF.XObject();
oFF.RsVisualizationTupleCollection.prototype._ff_c = "RsVisualizationTupleCollection";
...
oFF.RsDimensionMemberSelection = function() {};
oFF.RsDimensionMemberSelection.prototype = new oFF.RsVisualizationTupleCollection();
oFF.RsDimensionMemberSelection.prototype._ff_c = "RsDimensionMemberSelection";


oFF.RsVisualizationAxis = function() {};
oFF.RsVisualizationAxis.prototype = new oFF.RsVisualizationTupleCollection();
oFF.RsVisualizationAxis.prototype._ff_c = "RsVisualizationAxis";

oFF.RsVisualizationAxis.create = function(axisType, visualizationAxisType, cursorColumnsAxis, visualizationAxisByType)
{
	var result = new oFF.RsVisualizationAxis();
	result.m_axisType = axisType;
	result.m_visualizationAxisType = visualizationAxisType;
	return result;
};
oFF.RsVisualizationAxis.prototype.m_axisType = null;
oFF.RsVisualizationAxis.prototype.m_visualizationAxisType = null;
oFF.RsVisualizationAxis.prototype.getIntersectionSet = function(index)
{
	return null;
};
oFF.RsVisualizationAxis.prototype.getAxisType = function()
{
	return this.m_axisType;
};
oFF.RsVisualizationAxis.prototype.getVisualizationAxisType = function()
{
	return this.m_visualizationAxisType;
};

oFF.RsVisualizationResultSet = function() {};
oFF.RsVisualizationResultSet.prototype = new oFF.RsVisualizationResultCollection();
oFF.RsVisualizationResultSet.prototype._ff_c = "RsVisualizationResultSet";

oFF.RsVisualizationResultSet.prototype.m_columnGroupAxis = null;
oFF.RsVisualizationResultSet.prototype.m_rowGroupAxis = null;
oFF.RsVisualizationResultSet.prototype.m_columnCategoryAxis = null;
oFF.RsVisualizationResultSet.prototype.m_rowCategoryAxis = null;
oFF.RsVisualizationResultSet.prototype.m_columnDistributionAxis = null;
oFF.RsVisualizationResultSet.prototype.m_rowDistributionAxis = null;
oFF.RsVisualizationResultSet.prototype.m_columnAggregationAxis = null;
oFF.RsVisualizationResultSet.prototype.m_rowAggregationAxis = null;
oFF.RsVisualizationResultSet.prototype.m_dimensionMemberSelections = null;
oFF.RsVisualizationResultSet.prototype.getColumnGroupAxis = function()
{
	return this.m_columnGroupAxis;
};
oFF.RsVisualizationResultSet.prototype.getRowGroupAxis = function()
{
	return this.m_rowGroupAxis;
};
oFF.RsVisualizationResultSet.prototype.getColumnCategoryAxis = function()
{
	return this.m_columnCategoryAxis;
};
oFF.RsVisualizationResultSet.prototype.getRowCategoryAxis = function()
{
	return this.m_rowCategoryAxis;
};
oFF.RsVisualizationResultSet.prototype.getColumnDistributionAxis = function()
{
	return this.m_columnDistributionAxis;
};
oFF.RsVisualizationResultSet.prototype.getRowDistributionAxis = function()
{
	return this.m_rowDistributionAxis;
};
oFF.RsVisualizationResultSet.prototype.getColumnAggregationAxis = function()
{
	return this.m_columnAggregationAxis;
};
oFF.RsVisualizationResultSet.prototype.getRowAggregationAxis = function()
{
	return this.m_rowAggregationAxis;
};
oFF.RsVisualizationResultSet.prototype.create = function(rs, visualizationDefinition)
{
	var instance = new oFF.RsVisualizationResultSet();
	instance.setBaseResultSet(rs);
	instance.m_columnGroupAxis = oFF.RsVisualizationAxis.create(oFF.AxisType.COLUMNS, oFF.VisualizationAxisType.COLUMN_GROUPS, rs.getCursorColumnsAxis(), null);
	return instance;
};
oFF.RsVisualizationResultSet.prototype.getDimensionMemberSelections = function()
{
	return this.m_dimensionMemberSelections;
};
...
oFF.RsCursorAxisTupleElementContent.prototype.getParentDimensionMemberName = function()
{
	...
	if (oFF.notNull(dimension) && (dimension.getDimensionType() === oFF.DimensionType.ACCOUNT || dimension.getDimensionType() === oFF.DimensionType.MEASURE_STRUCTURE))
	{
		var dimensionMember = dimension.getStructureMember(this.m_parentDimensionMemberName);
		if (oFF.notNull(dimensionMember) && dimensionMember.getAliasName() !== null)
		{
			return dimensionMember.getAliasName();
		}
...
oFF.RsCursorAxisTupleElementContent.prototype.getParentDimensionMemberAliasOrName = function()
{
	...
	if (oFF.notNull(dimension))
	{
		...
		if (oFF.notNull(queryModel))
		{
			var aliasName = dimension.getConvenienceCommands().getMeasureMemberAliasByName(queryModel, this.m_parentDimensionMemberName);
			if (oFF.notNull(aliasName))
			{
				return aliasName;
			}
...
oFF.LovProcess.prototype.processSynchronization = function(syncType)
{
	...
	var valueHelp = this.m_lovProcessConfig.requiresValueHelpRequest();
...
{
	var queryManager = this.getQueryManager();
	var serializedValHelpRequest = null;
	if (oFF.notNull(queryManager))
	{
		this.prepareQuery();
		if (this.m_lovProcessConfig.requiresValueHelpRequest())
...
{
	var element = this.serializeToElementExt(modelFormat, null);
...
{
	var element = this.serializeToElementExt(modelFormat, capabilities);
	var content = oFF.XContent.createJsonObjectContent(modelFormat, element);
	return content;
};
...
oFF.ResultSetContainer.prototype.executeRuntime = function(syncType, listener, customIdentifier)
{
	...
	{
		var analytics = oFF.PrUtils.getStructureProperty(inaRequest, "Analytics");
		var definition = oFF.PrUtils.getStructureProperty(analytics, "Definition");
		var udhs = oFF.PrUtils.getListProperty(definition, "UniversalDisplayHierarchies");
		if (oFF.notNull(udhs))
		{
			var lbhToUdhConversionProperties = oFF.XStream.of(udhs).map( function(udh1){
				return udh1.asStructure();
			}.bind(this)).filter( function(udh2){
				return udh2.getBooleanByKeyExt("Active", false) && udh2.containsKey("LevelDimensionName") && udh2.containsKey("LevelHierarchyName");
			}.bind(this)).map( function(udh3){
	...
	if (this.getSyncState() === oFF.SyncState.OUT_OF_SYNC)
	{
		...
		for (var i = 0; i < size; i++)
		{
			...
			var rpcRequest = _function.getRpcRequest();
	...
	if (oFF.notNull(queryModel) && queryModel.isBlendingModel())
	{
		...
		while (sources.hasNext())
		{
			...
			if (source.isRemoteSource())
			{
				...
				{
					var syncType = manager.getResultSetSyncState() === oFF.SyncState.PROCESSING && source.isRemoteQueryPersistenceIdentifierUpToDate(blendingHost) ? oFF.SyncType.REGISTER : this.m_currentSyncType;
...
oFF.ResultSetContainer.prototype.onQueryExecuted = function(extResult, resultSetContainer, customIdentifier)
{
	...
	var queryManager = source.getQueryManager();
...
oFF.ResultSetContainer.prototype.processSynchronization = function(syncType)
{
	...
	if (oFF.isNull(this.m_rsDefQueryModel))
	{
		...
		var isExecuteAutoVariableSubmit = queryManager.isExecuteAutoVariableSubmit();
		if (oFF.notNull(queryModel))
		{
...
oFF.ResultSetContainer.prototype.requiresCustomHierarchyRecreate = function(errors)
{
	...
	var customHierarchyDefinitions = olapEnv.getCustomHierarchyRepository().getCustomHierarchyDefinitions();
	for (var i = 0; i < errors.size(); i++)
	{
		var error = errors.get(i);
		if (error.getCode() === oFF.ErrorCodes.CUSTOM_HIERARCHY_EXPIRED)
		{
			for (var j = 0; j < customHierarchyDefinitions.size(); j++)
			{
...
oFF.ResultSetContainer.prototype.getAbstractRendering = function(type, protocol)
{
	...
	else if (type.isTypeOf(oFF.SemanticBindingType.TABLE) && protocol.isTypeOf(oFF.ProtocolBindingType.SAC_TABLE_GRID))
	{
		...
		var tableDefinition = this.getQueryModel().getVisualizationManager().getOrCreateVisualisationDefinition("TABLE_DEFINITION", oFF.ProtocolBindingType.SAC_TABLE_GRID, oFF.SemanticBindingType.TABLE);
...
oFF.ResultSetContainer.prototype.validateQueryModel = function()
{
	var queryModel = this.getQueryModel();
...
{
	return this.m_dataCellContextResults.getKeysAsReadOnlyListOfString();
};
...
oFF.RsCursorAxis.prototype.addNextFieldMetadata = function(fieldName, fieldText, isVisible, presentationType, valueType, mimeType)
{
	...
	if (oFF.isNull(field))
	{
		...
		var queryManager = this.getCursorResultSet().getQueryManager();
	...
	var rsField = this.m_currentRsDimension.addRsField2(fieldName, fieldText, presentationType === oFF.PresentationType.UNDEFINED ? field.getPresentationType() : presentationType, valueType, field.getMimeType());
	if (isVisible)
	{
...
oFF.RsCursorResultSet.prototype.createRuntimeRsDefStructureMembersForDimension = function(dimension, runtimeStructureMemberProperties)
{
	...
	var outOfContextMembers = runtimeStructureMemberProperties.getOutOfContextMembers();
	...
	while (memberNamesIter.hasNext())
	{
		...
		var memberOrAliasName = oFF.XStringUtils.isNotNullAndNotEmpty(aliasName) ? aliasName : memberName;
		var minimumDrillState = runtimeStructureMemberProperties.getMinimumDrillState().getByKey(memberOrAliasName);
		var unsatisfiedRequiredDimensionNames = runtimeStructureMemberProperties.getUnsatisfiedRequiredDimensionNames().getByKey(memberOrAliasName);
		var availableFormulaExceptionIds = runtimeStructureMemberProperties.getAvailableFormulaExceptionIds().getByKey(memberOrAliasName);
		var nestedMemberQueryProperties = oFF.XHashMapByString.create();
		...
		var nestedAllRequiredDimensionProperties = oFF.XPair.create(nestedMemberQueryProperties, isOutOfContextValue);
...
oFF.RsCursorResultSet.prototype.collectRequiredDimensionsForAxis = function(axisType, reqDimensionNamesMap)
{
	...
	for (var i = 0; i < rsDimensions.size(); ++i)
	{
		...
		if (dimensionType === oFF.DimensionType.ACCOUNT || dimensionType === oFF.DimensionType.MEASURE_STRUCTURE)
		{
			var inUseMemberNames = this.collectDimensionMemberNamesInTuples(cursorAxis, rsDimension.getName());
			...
			{
				rsDefStructureMember = rsDefIterator.next();
				var rsDefStructureMemberAliasOrName = rsDefStructureMember.getAliasOrMemberName();
				var nestedRsAllRequiredDimensionProperties = rsDefStructureMember.getNestedAllRequiredDimensionProperties(null, null, true);
				var nestedRsMemberQueryProperties = nestedRsAllRequiredDimensionProperties.getFirstObject();
				var isOutOfContextValue = nestedRsAllRequiredDimensionProperties.getSecondObject();
				if (inUseMemberNames.contains(rsDefStructureMember.getName()) || allAvailableFormulaExceptionIds.contains(rsDefStructureMember.getName()))
				{
			...
			if (oFF.notNull(queryModel) && dimensionType === oFF.DimensionType.ACCOUNT)
			{
				...
				while (inUseMemberIterator.hasNext())
				{
					...
					if (oFF.notNull(structureMember))
					{
						...
						var nestedAllRequiredDimensionProperties = structureMember.getNestedAllRequiredDimensionProperties(cursorAxis.getRsAxisDef(), null, false);
						var nestedMemberQueryProperties = nestedAllRequiredDimensionProperties.getFirstObject();
...
oFF.RsCursorResultSet.prototype.collectDimensionMemberNamesInTuples = function(cursorAxis, dimensionName)
{
	...
	var rsDimension = oFF.XCollectionUtils.findFirst(cursorAxis.getRsDimensions(),  function(dimension){
		return oFF.XString.isEqual(dimension.getName(), dimensionName);
	}.bind(this));
	var allFields = rsDimension.getAllFields();
	var hierarchyKeyField = oFF.XCollectionUtils.findFirst(allFields,  function(field1){
	...
	{
		var flatKeyField = oFF.XCollectionUtils.findFirst(allFields,  function(field2){
...
oFF.RsCursorResultSet.prototype.validateCellMinimumDrillState = function(column, row)
{
	...
	var hasAnyRequiredDimensions = false;
	while (iterator.hasNext())
	{
		var dimName = iterator.next();
		var dimValidationContext = requiredDimensionNamesByMemberNameMap.getByKey(dimName);
		if (!dimValidationContext.isEmpty())
		{
			hasAnyRequiredDimensions = true;
			break;
		}
	}
	if (hasAnyRequiredDimensions)
	{
		...
		iterator = requiredDimensionNamesByMemberNameMap.getKeysAsIteratorOfString();
		while (iterator.hasNext())
		{
...
oFF.RsDefField.create = function(name, text, presentationType, valueType, rsDefDimension, mimeType)
{
...
oFF.RsCellDocumentIdAction.prototype.m_actionSyncType = null;
...
oFF.RsCellDocumentIdAction.prototype.releaseObject = function()
{
	this.m_actionSyncType = null;
	this.m_queryManager = null;
...
{
	this.m_actionSyncType = syncType;
	this.m_queryManager = queryManager;
...
oFF.RsCellDocumentIdAction.prototype.processSynchronization = function(syncType)
{
	try
	{
		if (this.m_queryManager.getModelCapabilities().supportsCellDocumentId())
		{
			...
			if (oFF.notNull(classicResultSet))
			{
				var cell = classicResultSet.getDataCell(this.m_indexInfo.getColumnIndex(), this.m_indexInfo.getRowIndex());
				if (this.isOperationValid(cell.getDocumentId()))
				{
					var documentsIdCollection = this.m_queryManager.getActiveResultSetContainer().getDocumentIdCollection();
					var documentId = documentsIdCollection.createDocumentId(classicResultSet.getColumnsAxis().getTupleAt(this.m_indexInfo.getColumnIndex()), classicResultSet.getRowsAxis().getTupleAt(this.m_indexInfo.getRowIndex()));
					documentId.getDocumentIdAction(cell.getDataCell()).setAction(this.m_action);
					this.m_queryManager.transferNewValues();
...
{
	var documentsInfo = this.m_queryManager.getQueryModel().getDocumentsInfo();
	var isValid = documentsInfo.getSupportsDocuments() === oFF.DocumentsSupportType.READ_WRITE;
	if (!isValid)
	{
...
oFF.RsCellDocumentIdAction.prototype.onQueryExecuted = function(extResult, resultSetContainer, customIdentifier)
{
	...
	if (extResult.isValid())
	{
		...
		if (oFF.notNull(resultSet) && resultSet.getState().hasData())
		{
			...
			dataAreaCommand = this.m_queryManager.createDataAreaCommandDocSave();
			dataAreaCommand.processCommand(this.m_actionSyncType, this, null);
			this.m_isCellDocIdSet = true;
		}
		else
		{
			hasExecutionFailed = true;
			var messages = this.m_queryManager.getActiveResultSetContainer().getMessages();
			if (messages.size() > 0)
			{
	...
	{
		dataAreaCommand = this.m_queryManager.createDataAreaCommandDocReset();
		dataAreaCommand.processCommand(this.m_actionSyncType, this, null);
	}
};
...
oFF.RsVizVisualizationContainer.prototype.processExecution = function(syncType, listener, customIdentifier)
{
	...
	if (!rsContainer.isCursorResultSetAvailable())
	{
		...
		{
			rsContainer.processExecution(syncType, this, syncType);
		}
	}
	else
	{
...
{
	var isBeingReset = false;
	if (this.m_pendingReset)
	{
		var curSyncState = this.getSyncState();
...
oFF.RsVizVisualizationContainer.prototype.processSynchronization = function(syncType)
{
	...
	{
		this.m_timerId = oFF.XTimeout.timeout(10,  function(){
			try
			{
...
oFF.RsVizVisualizationContainer.prototype.onQueryExecuted = function(extResult, resultSetContainer, customIdentifier)
{
	if (extResult.isValid())
	{
		this.processSynchronization(customIdentifier);
	}
	else
	{
...
oFF.RsAxisTupleElement.prototype.getDrillPath = function()
{
	...
	var hasSidPresentation = this.getQueryModel().getModelCapabilities().supportsSidPresentation();
	for (var i = 0; i <= this.m_tupleElementPosition; i++)
	{
		...
		var overwriteKeyField = hasSidPresentation;
		if (hasSidPresentation)
		{
		...
		if (overwriteKeyField)
		{
			element.setFieldName(dimension.getHierarchyNavigationField().getName());
		}
		...
		if (dimension.getDimensionType() === oFF.DimensionType.ACCOUNT || dimension.getDimensionType() === oFF.DimensionType.MEASURE_STRUCTURE)
		{
			...
			for (var i = 0; i < structureMembers.size(); i++)
			{
				...
				if (oFF.notNull(structureMember) && !structureMember.isReleased())
				{
					...
					var nestedMemberQueryProperties = this.getNestedAllRequiredDimensionProperties(context, rsDefAxisDimension, structureMember, inUseFormulaExceptionIds);
					var rsStructureMember = oFF.RsDefStructureMember.create(placeholderName, structureMember.getAliasName(), nestedMemberQueryProperties, structureMember.getAvailableFormulaExceptionIds(context), context, dimension.getName(), this.m_cacheKey);
...
oFF.RsDefDimension.prototype.addRsField2 = function(fieldName, fieldText, presentationType, valueType, mimeType)
{
	var rsField = oFF.RsDefField.create(fieldName, fieldText, presentationType, valueType, this, mimeType);
...
oFF.RsDefDimension.prototype.getNestedAllRequiredDimensionProperties = function(context, rsDefAxisDimension, structureMember, inUseFormulaExceptionIds)
{
	if (!context.getSession().hasFeature(oFF.FeatureToggleOlap.SFX_MINIMUM_DRILL_STATE))
	{
		return null;
	...
	if (oFF.notNull(rsDefAxisDimension))
	{
		...
		{
			return rsDefAxisStructureMember.getNestedAllRequiredDimensionProperties(context, null, checkInUsemeasures);
		}
	}
	return structureMember.getNestedAllRequiredDimensionProperties(context, null, checkInUsemeasures);
};
...
{
	var text = oFF.RsDefModelComponent.prototype.getText.call( this );
	...
	else if (this.getOlapEnv() !== null && this.getApplication() !== null && this.getSession() !== null && this.getSession().hasFeature(oFF.FeatureToggleOlap.MEMBER_OVERRIDE_TEXTS))
	{
		...
		{
			var olapEnvCube = this.getOlapEnv().getCubeContainer(this.m_cacheKey);
...
oFF.RsDefStructureMember.prototype.getNestedAllRequiredDimensionProperties = function(context, messageManager, checkInUsemeasures)
{
...
oFF.RsDefStructureMember.prototype.getSharedOverrideText = function()
{
	...
	{
		var olapEnvCube = this.getOlapEnv().getCubeContainer(this.m_dimensionCacheKey);
		if (oFF.notNull(olapEnvCube))
		{
			var storageName = this.getQueryModel().getQueryManager().getClientQueryObjectStorageName();
			var datasetId = this.getQueryModel().getDatasetId();
...
{
	var obj = new oFF.RsCellDocumentIdCreateAction();
...
{
	var isValid = oFF.XStringUtils.isNullOrEmpty(cellDocumentId);
	if (!isValid)
	{
...
{
	var obj = new oFF.RsCellDocumentIdDeleteAction();
...
{
	var isValid = oFF.XStringUtils.isNotNullAndNotEmpty(cellDocumentId);
	if (!isValid)
	{
...
{
	var obj = new oFF.RsCellDocumentIdGetAction();
...
oFF.RsCellDocumentIdGetAction.prototype.processSynchronization = function(syncType)
{
	try
	{
		if (this.m_queryManager.getModelCapabilities().supportsCellDocumentId())
		{
			var resultSet = this.m_queryManager.getCursorResultSet();
			if (oFF.notNull(resultSet) && resultSet.getState().hasData())
			{
				this.m_cellDocumentId = resultSet.getDataCell(this.m_indexInfo.getColumnIndex(), this.m_indexInfo.getRowIndex()).getDocumentId();
				this.m_isCellDocIdSet = true;
