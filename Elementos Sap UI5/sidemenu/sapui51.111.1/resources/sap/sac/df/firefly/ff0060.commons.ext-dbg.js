...
{
	var dateRange = new oFF.XDateRange();
...
{
	var clone = oFF.XDateRange.create(this.m_dateTimeProvider);
...
oFF.XDateRange.prototype.compute = function()
{
	if (oFF.isNull(this.m_from) || oFF.isNull(this.m_to))
	{
		...
		{
			var timeInMillis = oFF.notNull(this.m_dateTimeProvider) ? this.m_dateTimeProvider.getCurrentTimeInMilliseconds() : oFF.XSystemUtils.getCurrentTimeInMilliseconds();
			this.m_from = oFF.XGregorianCalendar.createWithTimeZone(timeZone);
			...
			if (oFF.notNull(this.m_offset))
			{
				var offsetField = this.mapGranularityToField(this.m_offset.getFirstObject());
				var offset = this.m_offset.getSecondObject().getInteger();
...
oFF.XDateRange.prototype.computeGranularity = function()
{
	if (oFF.notNull(this.m_granularity) && this.m_granularity !== oFF.DateRangeGranularity.DAY)
	{
		if (oFF.notNull(this.m_from))
		{
			...
			{
				var fromMonth = this.m_from.get(oFF.DateConstants.MONTH);
				var firstMonthInQuarter = fromMonth <= 3 ? 1 : fromMonth <= 6 ? 4 : fromMonth <= 9 ? 7 : 10;
		...
		if (!this.m_upToCurrentPeriod && oFF.notNull(this.m_to))
		{
			...
			if (this.m_granularity.isQuarter())
			{
				var toMonth = this.m_to.get(oFF.DateConstants.MONTH);
				var lastMonthInQuarter = toMonth <= 3 ? 3 : toMonth <= 6 ? 6 : toMonth <= 9 ? 9 : 12;
...
{
	var field = this.mapGranularityToField(granularity);
	if (field !== -1)
	{
		this.compute();
		if (oFF.notNull(this.m_from))
...
{
	var obj = new oFF.XAmPmFormatToken();
	obj.m_size = 2;
...
oFF.XAmPmFormatToken.prototype.setDateTimeElementValue = function(dateTime, dateTimeString, begin)
{
	...
	{
		var hourOfDay = dateTime.getHourOfDay();
...
{
	var obj = new oFF.XMonthStringToken();
	obj.setup();
	return obj;
};
...
{
	this.m_monthStrings = oFF.XArrayOfString.create(12);
...
{
	var dateElementString = oFF.XString.substring(dateTimeString, begin, begin + 3);
	for (var i = 0; i < this.m_monthStrings.size(); i++)
	{
...
{
	var monthOfYear = dateTime.getMonthOfYear();
...
oFF.XTimeZoneFormatToken.prototype.setDateTimeElementValue = function(dateTime, dateTimeString, begin)
{
	...
	{
		var timeZone = this.getTimeZone(dateTimeString, newBegin);
...
oFF.XCaseUtils = {
	...
	{
			var convertedText = oFF.XCaseUtils._processText(text,  function(tmpToken, newTokenList){
			var modifiedToken = oFF.XString.toLowerCase(tmpToken);
	...
	_processText:function(textToProcess, tokenFunction, joinStr)
	{
		...
		var textTokens = oFF.XCaseUtils._tokenizeText(textToProcess);
		var newTokens = oFF.XListOfString.create();
		oFF.XCollectionUtils.forEachString(textTokens,  function(tmpToken){
			var modifiedToken = tokenFunction(tmpToken, newTokens);
	...
	{
			var buffer = oFF.XStringBuffer.create();
		var textTokens = oFF.XListOfString.create();
		var size = oFF.XString.size(text);
		var mode = 0;
		for (var i = 0; i < size; i++)
		{
			var charAt = oFF.XString.getCharAt(text, i);
			var newMode = 0;
...
oFF.XNumberFormatter = {
	...
	{
			var settings = oFF.XNumberFormatterSettingsFactory.getInstance().createFromString(format);
		return oFF.XNumberFormatter.formatDouble(value, settings);
	},
	formatDoubleToStringUsingSettings:function(value, numberFormatterSettings)
	{
			var settings = new oFF.XNumberFormatterSettings();
	...
	formatDoubleToStringUsingSettingsWithCurrency:function(value, numberFormatterSettings, currencyText, scaleTexts, defaultExcelFormat)
	{
			if (oFF.isNull(currencyText))
		{
			throw oFF.XException.createIllegalArgumentException("The currency must not be null");
		}
		var formattedValue = oFF.XNumberFormatter.formatDoubleToStringUsingSettings(value, numberFormatterSettings);
		var excelFormat = oFF.XNumberFormatter.adjustExcelDecimalPlace(defaultExcelFormat, numberFormatterSettings);
		var currencyFormatSettings = numberFormatterSettings.getCurrencyFormatSettings();
		if (oFF.notNull(currencyFormatSettings))
		{
			var currencyPresentation = currencyText.getFirstString();
			if (oFF.XStringUtils.isNotNullAndNotEmpty(currencyPresentation) && currencyFormatSettings.currencyPresentationHasSpace())
			{
				var spaceIsPrefixed = currencyFormatSettings.getCurrencyPresentationPosition() > currencyFormatSettings.getValuePosition();
				currencyPresentation = oFF.XNumberFormatter.concatSpace(currencyPresentation, spaceIsPrefixed);
			}
			var scaleFormatTuple = oFF.XNumberFormatter.getScaleText(numberFormatterSettings.getScaleFactor(), numberFormatterSettings.getScaleFormat(), scaleTexts);
			var scaleText = scaleFormatTuple.getName();
			var excelScale = scaleFormatTuple.getValue();
			if (oFF.XStringUtils.isNotNullAndNotEmpty(scaleText) && currencyFormatSettings.scaleTextHasSpace())
			{
				var spaceIsPrefixed2 = currencyFormatSettings.getScaleTextPosition() > currencyFormatSettings.getValuePosition();
			...
			var positions = currencyFormatSettings.getValuePosition() * 100 + currencyFormatSettings.getScaleTextPosition() * 10 + currencyFormatSettings.getCurrencyPresentationPosition();
			switch (positions)
			{
				case 12:
					formattedValue = oFF.XStringUtils.concatenate5(prefix, formattedValue, scaleText, currencyPresentation, suffix);
					excelFormat = oFF.XNumberFormatter.buildExcelFormatString(prefix, "", "", excelFormat, excelScaleText, currencyPresentation, suffix, numberFormatterSettings);
					break;

				case 21:
					formattedValue = oFF.XStringUtils.concatenate5(prefix, formattedValue, currencyPresentation, scaleText, suffix);
					excelFormat = oFF.XNumberFormatter.buildExcelFormatString(prefix, "", "", excelFormat, currencyPresentation, excelScaleText, suffix, numberFormatterSettings);
					break;

				case 102:
					formattedValue = oFF.XStringUtils.concatenate5(prefix, scaleText, formattedValue, currencyPresentation, suffix);
					excelFormat = oFF.XNumberFormatter.buildExcelFormatString(prefix, "", excelScaleText, excelFormat, currencyPresentation, "", suffix, numberFormatterSettings);
					break;

				case 120:
					formattedValue = oFF.XStringUtils.concatenate5(prefix, currencyPresentation, formattedValue, scaleText, suffix);
					excelFormat = oFF.XNumberFormatter.buildExcelFormatString(prefix, currencyPresentation, "", excelFormat, excelScaleText, "", suffix, numberFormatterSettings);
					break;

				case 201:
					formattedValue = oFF.XStringUtils.concatenate5(prefix, scaleText, currencyPresentation, formattedValue, suffix);
					excelFormat = oFF.XNumberFormatter.buildExcelFormatString(prefix, excelScaleText, currencyPresentation, excelFormat, "", "", suffix, numberFormatterSettings);
					break;

				case 210:
					formattedValue = oFF.XStringUtils.concatenate5(prefix, currencyPresentation, scaleText, formattedValue, suffix);
					excelFormat = oFF.XNumberFormatter.buildExcelFormatString(prefix, currencyPresentation, excelScaleText, excelFormat, "", "", suffix, numberFormatterSettings);
					break;
		...
		{
			excelFormat = oFF.XNumberFormatter.buildExcelFormatString("", "", "", excelFormat, "", "", "", numberFormatterSettings);
		}
	...
	adjustExcelDecimalPlace:function(defaultExcelFormat, numberFormatterSettings)
	{
		...
		var indexOfDecimal = oFF.XString.indexOf(defaultExcelFormat, ".");
		var numberOfZerosNeeded = oFF.XMath.min(numberFormatterSettings.getMaxDigitsRight(), numberFormatterSettings.getRightPad());
		...
		if (indexOfDecimal >= 0)
		{
			var lastIndex = numberOfZerosNeeded > 0 ? indexOfDecimal + 1 : indexOfDecimal;
			result = oFF.XStringUtils.rightPad(oFF.XString.substring(result, 0, lastIndex), "0", numberOfZerosNeeded);
		}
		return result;
	},
	buildExcelFormatString:function(prefix1, prefix2, prefix3, excelFormatString, suffix1, suffix2, suffix3, numberFormatterSettings)
	{
			var newPrefix = "";
		var newSuffix = "";
		var result = "";
		var prefixNoSign = oFF.XString.replace(prefix1, "+", "");
		prefixNoSign = oFF.XString.replace(prefixNoSign, "-", "");
		var suffixNoSign = oFF.XString.replace(suffix3, "+", "");
		...
		if (numberFormatterSettings.getSignPresentation() === oFF.SignPresentation.BRACKETS)
		{
			...
			var positive = oFF.XStringUtils.concatenate3(newPrefix, excelFormatString, newSuffix);
			var negative = oFF.XStringUtils.concatenate5(newPrefix, "(", excelFormatString, ")", newSuffix);
		...
		else
		{
			...
			var tailSuffix = oFF.XNumberFormatter.wrapXlsString(suffixNoSign, "", "");
			var resultWithoutTail = oFF.XStringUtils.concatenate3(newPrefix, excelFormatString, newSuffix);
			if (numberFormatterSettings.getSignPresentation() === oFF.SignPresentation.BEFORE_NUMBER)
			{
				var positive1 = oFF.XStringUtils.concatenate3("+", resultWithoutTail, tailSuffix);
				var negative1 = oFF.XStringUtils.concatenate3("-", resultWithoutTail, tailSuffix);
			...
			{
				var positive2 = oFF.XStringUtils.concatenate3(resultWithoutTail, "+", tailSuffix);
				var negative2 = oFF.XStringUtils.concatenate3(resultWithoutTail, "-", tailSuffix);
	...
	formatDouble:function(value, numberFormatterSettings)
	{
			var valueString = oFF.XNumberFormatter.normalizeScientificFormat(oFF.XDouble.convertToString(value), oFF.XNumberFormatter.charDot);
		if (numberFormatterSettings.getScaleFactor() !== null)
		{
			var scaleProcessingStr = valueString;
			var addBackNegative = false;
			...
			var scaleFactor = numberFormatterSettings.getScaleFactor().getInteger();
			if (scaleFactor < 0)
			{
		...
		var rightExpression = "";
		var isRight = false;
		var prefixSign = numberFormatterSettings.getPrefix();
		...
		var maxDigitsRight = numberFormatterSettings.getMaxDigitsRight();
		if (maxDigitsRight !== 0)
		{
			if (maxDigitsRight > 0 && rightExpressionLength > maxDigitsRight)
			{
				var lastDigit = oFF.XNumberFormatter.charToInt(oFF.XString.getCharAt(rightExpression, maxDigitsRight - 1));
				var roundingBase = oFF.XNumberFormatter.charToInt(oFF.XString.getCharAt(rightExpression, maxDigitsRight));
		...
		{
			var decimalSeparator = oFF.XStringUtils.isNotNullAndNotEmpty(numberFormatterSettings.getDecimalSeparator()) ? numberFormatterSettings.getDecimalSeparator() : ".";
	...
	moveDecimalLeft:function(number, leftShiftCount)
	{
		...
		var length = oFF.XString.size(valueString);
		var decimalPosition = oFF.XString.indexOf(valueString, ".");
		if (decimalPosition < 0)
		{
			decimalPosition = length;
		}
		var splitAroundDecimal = oFF.XNumberFormatter.splitAround(valueString, ".");
		var newBeforeDecimal;
		var newAfterDecimal;
		if (leftShiftCount >= decimalPosition)
		{
			var withoutDecimal = oFF.XStringUtils.concatenate2(splitAroundDecimal.getFirstString(), splitAroundDecimal.getSecondString());
			newBeforeDecimal = "0";
			newAfterDecimal = oFF.XStringUtils.leftPad(withoutDecimal, "0", leftShiftCount - decimalPosition);
		}
		else
		{
			var newDecimalPosition = decimalPosition - leftShiftCount;
	...
	moveDecimalRight:function(number, rightShiftCount)
	{
		...
		var splitAroundDecimal = oFF.XNumberFormatter.splitAround(valueString, ".");
		var beforeDecimal = splitAroundDecimal.getFirstString();
		var afterDecimal = splitAroundDecimal.getSecondString() === null ? "" : splitAroundDecimal.getSecondString();
		...
		{
			var splitIndex = oFF.XString.size(afterDecimal) - rightShiftCount;
			var first = oFF.XString.substring(afterDecimal, 0, splitIndex);
			var second = oFF.XString.substring(afterDecimal, splitIndex, oFF.XString.size(afterDecimal));
	...
	{
			var length = oFF.XString.size(numericString);
		var numTailing0s = 0;
		var nonZeroFound = false;
		for (var i = length - 1; i >= 0 && !nonZeroFound; i--)
		{
			var c = oFF.XString.getCharAt(numericString, i);
	...
	stripLeadingZeros:function(numericString)
	{
		...
		var numLeading0s = 0;
		var nonZeroFound = false;
		for (var i = 0; i < length && !nonZeroFound; i++)
		{
			var c = oFF.XString.getCharAt(numericString, i);
			if (c === oFF.XNumberFormatter.char0 || c === oFF.XNumberFormatter.charDot)
			{
				numLeading0s++;
			}
			else
	...
	splitAround:function(str, splitChar)
	{
			var first = str;
		var second = null;
		var splitIndex = oFF.XString.indexOf(str, splitChar);
		if (splitIndex >= 0)
		{
	...
	{
			var scaleText = "";
		var excelScaleFormat = "";
		if (oFF.notNull(scaling))
		{
			var scalingFactor = scaling.getInteger();
			if (scalingFactor < 0)
			{
				var numZeros = (scalingFactor + 1) * -1;
				scaleText = oFF.XStringUtils.concatenate3("(0.", oFF.XStringUtils.leftPad("1", "0", numZeros), ")");
			}
			else
			{
				var scaleFormatTuple = oFF.XNumberFormatter.getDefaultScaleFormatTuple(scalingFactor);
				var defaultScaleText = scaleFormatTuple.getFirstString();
				excelScaleFormat = scaleFormatTuple.getSecondString();
				var scaleFormatName = oFF.isNull(scaleFormat) ? oFF.ScaleFormat.SHORT.getName() : scaleFormat.getName();
				var key = oFF.XStringUtils.concatenate3(scaleFormatName, "_", oFF.XInteger.convertToString(scalingFactor));
	...
	getDefaultScaleFormatTuple:function(scalingFactor)
	{
		...
		switch (scalingFactor)
		{
			...
			default:
				var billionsString = oFF.XStringUtils.rightPad("1", "0", scalingFactor - 9);
	...
	standardizeFormatString:function(formatStr, isBW, dateBased)
	{
			var result = "";
		if (!dateBased && isBW)
		{
...
{
	var keyIterator = this.m_consumers.getKeysAsIteratorOfString();
	while (keyIterator.hasNext())
	{
		var consumerKey = keyIterator.next();
		var consumer = this.m_consumers.getByKey(consumerKey);
...
{
	var consumerIterator = this.m_consumers.getIterator();
	while (consumerIterator.hasNext())
	{
		var consumer = consumerIterator.next();
...
{
	var dateFormatter = new oFF.XDateTimeFormatter();
...
{
	var dateTime = oFF.XDateTime.createEmptyDateTime();
	var currentIndex = 0;
	for (var i = 0; i < this.tokens.size(); i++)
	{
		...
		var separatorSize = (i < this.separators.size()) ? oFF.XString.size(this.separators.get(i).getString()) : 0;
...
{
	var dateTime = oFF.XDateTime.createWithYearMonthDay(date.getYear(), date.getMonthOfYear(), date.getDayOfMonth());
...
{
	var result = oFF.XStringBuffer.create().append("XError: ").append(this.m_text);
...
oFF.XCurrencyFormatSettings.prototype.m_scaleTextHasSpace = false;
oFF.XCurrencyFormatSettings.prototype.m_currencyPresentationHasSpace = false;
...
oFF.XGregorianCalendar.prototype.add = function(field, amountIn)
{
	...
	else if (field === oFF.DateConstants.MONTH || field === oFF.DateConstants.QUARTER)
	{
		year = this.internalGet(oFF.DateConstants.YEAR);
		var month = this.internalGet(oFF.DateConstants.MONTH);
		var monthAmount = field === oFF.DateConstants.QUARTER ? amount * 3 : amount;
		var months = oFF.XLong.convertToInt(year * 12 + month + monthAmount);
		...
		var maxDayOfMonth = this.getDaysInMonth(newYear, oFF.XMath.max(newMonth, 1));
...
oFF.XGregorianCalendar.prototype.rollAmount = function(field, amount)
{
	...
	{
		this.rollField(oFF.DateConstants.MONTH, field === oFF.DateConstants.QUARTER ? amount * 3 : amount);
		this.clampDate();
...
{
	var daysInMonth = oFF.DateConstants.DAYS_IN_MONTH.get(onBasedMonth - 1);
...
{
	return new oFF.XNumberFormatterSettings();
};
oFF.XNumberFormatterSettingsFactoryImpl.prototype.createFromString = function(numberFormat)
{
	...
	for (var i = 0; i < oFF.XString.size(numberFormat); i++)
	{
		var currentChar = oFF.XString.getCharAt(numberFormat, i);
...
{
	var target = oFF.isNull(nullableTarget) ? this.create2() : nullableTarget;
	var source = oFF.isNull(nullableSource) ? this.create2() : nullableSource;
	var settings = this.create();
	...
	var currencyFormatSettings = source.getCurrencyFormatSettings() !== null ? source.getCurrencyFormatSettings() : target.getCurrencyFormatSettings();
	settings.setCurrencyFormatSettings(this.copy(currencyFormatSettings));
...
oFF.XNumberFormatterSettingsFactoryImpl.prototype.copy = function(source)
{
	if (oFF.isNull(source))
	{
		return null;
	}
	var settings = oFF.XCurrencyFormatSettings.create();
	settings.setCurrencyPresentation(source.getCurrencyPresentation());
	settings.setCurrencyPresentationHasSpace(source.currencyPresentationHasSpace());
	settings.setCurrencyPresentationPosition(source.getCurrencyPresentationPosition());
	settings.setScaleTextHasSpace(source.scaleTextHasSpace());
	settings.setScaleTextPosition(source.getScaleTextPosition());
	settings.setValuePosition(source.getValuePosition());
	return settings;
};
...
oFF.DateConstants.FIELD_COUNT = 18;
...
oFF.XTime.createEmptyTime = function()
{
...
oFF.XDate.createEmptyDate = function()
{
...
oFF.XDate.createWithMillisAndTimezone = function(timeInMillis, timeZone)
{
	...
	var date = oFF.XDate.createDateWithValues(year, month, day);
...
oFF.XDateTime.createWithTimezoneAndMillis = function(timeInMillis, timeZone)
{
	...
	var second = calendar.get(oFF.DateConstants.SECOND);
	var millisecond = calendar.get(oFF.DateConstants.MILLISECOND);
	var dateTime = oFF.XDateTime.createDateTimeWithValues(year, month, day, hour, minute, second, millisecond);
	dateTime.setTimeZone(timeZone);
	return dateTime;
};
oFF.XDateTime.createEmptyDateTime = function()
{
	var result = new oFF.XDateTime();
	result.m_date = oFF.XDate.createEmptyDate();
	result.m_time = oFF.XTime.createEmptyTime();
	return result;
};
...
{
	var result = oFF.XDateTime.createEmptyDateTime();
	result.setYear(year);
...
oFF.XDateTime.createDateTimeSafe = function(dateTime)
{
	...
	if (oFF.XStringUtils.isNotNullAndNotEmpty(dateTime))
	{
		...
		if (oFF.notNull(date))
		{
			...
			if (oFF.isNull(time))
			{
				time = oFF.XTime.createEmptyTime();
			}
			result.m_date = date;
			result.m_time = time;
		}
...
oFF.XEnvironment.createCopy = function(env)
{
	...
	var allLockedProperties = env.getAllLockableProperties();
	var lockedProperties = allLockedProperties.getValuesAsReadOnlyListOfString();
	for (var k = 0; k < lockedProperties.size(); k++)
	{
...
{
	var values = this.m_lockableProperties.getValuesAsReadOnlyListOfString();
...
oFF.XEnvironment.prototype.getAllPrefixMatches = function(prefix)
{
	...
	for (var i = 0; i < keys.size(); i++)
	{
		...
		{
			var value = this.m_properties.getByKey(theKey);
			var pair = oFF.XNameValuePair.create(theKey, value);
			result.add(pair);
		}
	}
	return result;
