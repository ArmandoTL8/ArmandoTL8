...
oFF.QsRestDp.prototype.setupDp = function(reportingApp, definition)
{
	...
	var name = definition.getStringByKey(oFF.UiQuasarConstants.QSA_NAME);
	var endpointUrl = definition.getStringByKey(oFF.UiQuasarConstants.QSA_ENDPOINT);
...
oFF.QsOlapDp.prototype.setupDp = function(reportingApp, definition)
{
	...
	var dataSource = definition.getStringByKey(oFF.UiQuasarConstants.QSA_DATA_SOURCE);
	...
	var name = definition.getStringByKey(oFF.UiQuasarConstants.QSA_NAME);
	this.setName(name);
	var system = definition.getStringByKey(oFF.UiQuasarConstants.QSA_SYSTEM);
	...
	var vizDef = definition.getStructureByKey(oFF.UiQuasarConstants.QSA_VIZ_DEF);
	...
	if (modelFormat === oFF.QModelFormat.UQM)
	{
		...
		var variables = definition.getListByKey(oFF.UiQuasarConstants.VARIABLES);
		this.setVariables(variables);
		var globalDef = definition.getStructureByKey(oFF.UiQuasarConstants.QSA_GLOBAL_DEF);
		var globalDefs = definition.getStructureByKey(oFF.UiQuasarConstants.QSA_GLOBAL_DEFS);
		if (oFF.isNull(globalDef) && oFF.notNull(globalDefs))
		{
			globalDef = globalDefs;
		}
		this.setGlobalDef(globalDef);
		var ffQuery = definition.getStringByKey(oFF.UiQuasarConstants.QSA_FF_QUERY);
...
oFF.QsOlapDp.prototype.processInit = function()
{
	...
	if (this.m_modelFormat === oFF.QModelFormat.UQM)
	{
		...
		if (oFF.notNull(this.m_globalObjects))
		{
			...
			var globalStructure = uqm.putNewStructure(oFF.UiQuasarConstants.QSA_GLOBAL_OBJECTS);
...
oFF.QuasarEngine.prototype.initializeUi = function(uiManager)
{
	...
	var genesis = oFF.UiGenesis.create(root);
...
oFF.QuasarEngine.prototype.renderDocument = function()
{
	...
	else
	{
		...
		else
		{
			...
			{
				var type = document.getElementTypeByKey(oFF.UiQuasarConstants.QSA_MASTER_SYSTEM);
				...
				{
					var masterStructure = document.getStructureByKey(oFF.UiQuasarConstants.QSA_MASTER_SYSTEM);
					this.m_masterSystemName = masterStructure.getStringByKey(oFF.UiQuasarConstants.QSA_NAME);
					var sharedConnections = masterStructure.getIntegerByKeyExt(oFF.UiQuasarConstants.QSA_MAX_SHARED_CONNECTIONS, -1);
			...
			var globalObjects = document.getStructureByKey(oFF.UiQuasarConstants.QSA_GLOBAL_OBJECTS);
...
{
	var floatingControls = document.getListByKey(oFF.UiQuasarConstants.QSA_FLOATING_CONTROLS);
	if (oFF.notNull(floatingControls))
	{
		for (var h = 0; h < floatingControls.size(); h++)
		{
			var floatingControlStruct = floatingControls.getStructureAt(h);
			this.parseControl(floatingControlStruct, null);
		}
	}
	var docElement = document.getStructureByKey(oFF.UiQuasarConstants.QSA_CONTENT);
	if (oFF.isNull(docElement))
	{
		docElement = document;
	}
	var uiInnerContent = this.parseControl(docElement, itemUiType);
	var dpContent = document.getListByKey(oFF.UiQuasarConstants.QSA_DATA_PROVIDERS);
	...
	var dpStructure2 = document.getStructureByKey(oFF.UiQuasarConstants.QSA_DATA_PROVIDER);
	if (oFF.notNull(dpStructure2))
	{
		this.parseDataprovider(dpStructure2);
	}
	var bindings = document.getListByKey(oFF.UiQuasarConstants.QSA_BINDINGS);
...
oFF.QuasarEngine.prototype.parseControl = function(docElement, itemUiType)
{
	...
	var uiType = oFF.UiType.lookupUiType(docElement.getStringByKey(oFF.UiQuasarConstants.QSA_CTYPE));
	if (oFF.isNull(uiType))
	{
		uiType = itemUiType;
	}
	var customControlString = docElement.getStringByKey(oFF.UiQuasarConstants.QSA_CUSTOM_CONTROL);
	var styleClass = oFF.UiStyleClass.lookup(docElement.getStringByKey(oFF.UiQuasarConstants.QSA_STYLE));
	if (oFF.isNull(styleClass) && oFF.notNull(this.m_globalStyleClass))
	{
		styleClass = this.m_globalStyleClass;
	}
	var theId = docElement.getStringByKey(oFF.UiQuasarConstants.QSA_ID);
	var name = docElement.getStringByKey(oFF.UiQuasarConstants.QSA_NAME);
	...
	if (oFF.notNull(uiType))
	{
				...
				else if (oFF.UiAggregation.lookup(key) !== null)
				{
					...
					if (uiInnerContent.hasAggregation(tmpAggrDef))
					{
						...
						if (oFF.notNull(aggregationItems) && aggregationItems.size() > 0)
						{
							...
							var isItemsAggr = tmpAggrDef === oFF.UiAggregation.ITEMS;
							...
							if (isItemsAggr)
							{
								var selectedName = docElement.getStringByKey(oFF.UiQuasarConstants.QSA_SELECTED_NAME);
		...
		var theMenuStruct = docElement.getStructureByKey(oFF.UiProperty.MENU.getName());
		if (oFF.notNull(theMenuStruct))
		{
			var retContextMenu = this.parseDocument(theMenuStruct, null);
			uiInnerContent.setMenu(retContextMenu);
		}
		var theNavList = docElement.getStructureByKey(oFF.UiProperty.NAV_LIST.getName());
		if (oFF.notNull(theNavList))
		{
			var retContextNavList = this.parseDocument(theNavList, null);
			uiInnerContent.setNavList(retContextNavList);
		}
		var theFixedNavList = docElement.getStructureByKey(oFF.UiProperty.FIXED_NAV_LIST.getName());
		if (oFF.notNull(theFixedNavList))
		{
			var retContextFixedNavList = this.parseDocument(theFixedNavList, null);
	...
	{
		var ctype = docElement.getStringByKey(oFF.UiQuasarConstants.QSA_CTYPE);
...
{
	var docSupportedPlatformsStr = docElement.getStringByKey(oFF.UiQuasarConstants.QSA_SUPPORTED_PLATFORMS);
...
{
	var dpTypeStr = docElement.getStringByKey(oFF.UiQuasarConstants.QSA_TYPE);
	var dpType = oFF.QsDpType.lookup(dpTypeStr);
...
oFF.QuasarEngine.prototype.parseBinding = function(docElement, isNeo)
{
	...
	if (docElement.isStructure())
	{
		...
		{
			var cacheIdentifier1 = bindingStructure.getStringByKey(oFF.UiQuasarConstants.QSA_SOURCE);
			var cacheIdentifier2 = bindingStructure.getStringByKey(oFF.UiQuasarConstants.QSA_TYPE);
			cacheId = oFF.XStringUtils.concatenate2(cacheIdentifier1, cacheIdentifier2);
		}
		binding.setCacheIdentifier(cacheId);
		var source = bindingStructure.getStringByKey(oFF.UiQuasarConstants.QSA_SOURCE);
		binding.setSource(source);
		var target = bindingStructure.getStringByKey(oFF.UiQuasarConstants.QSA_TARGET);
		if (oFF.notNull(target))
		{
			binding.setTarget(target);
		}
		var bindingType = bindingStructure.getStringByKey(oFF.UiQuasarConstants.QSA_TYPE);
...
oFF.QuasarEngine.prototype.sendEventWithNameNew = function(event, eventDef)
{
	if (oFF.isNull(eventDef))
	{
		throw oFF.XException.createRuntimeException("Missing event! Please specify an event!");
...
{
	this.sendEventWithNameNew(event, oFF.UiEvent.ON_MOVE);
};
oFF.QuasarEngine.prototype.onMoveStart = function(event)
{
	this.sendEventWithNameNew(event, oFF.UiEvent.ON_MOVE_START);
};
oFF.QuasarEngine.prototype.onMoveEnd = function(event)
{
	this.sendEventWithNameNew(event, oFF.UiEvent.ON_MOVE_END);
};
oFF.QuasarEngine.prototype.onResize = function(event)
{
	this.sendEventWithNameNew(event, oFF.UiEvent.ON_RESIZE);
};
...
{
	this.sendEventWithNameNew(event, oFF.UiEvent.ON_DROP);
};
...
{
	this.sendEventWithNameNew(event, oFF.UiEvent.ON_KEY_DOWN);
};
oFF.QuasarEngine.prototype.onKeyUp = function(event)
{
	this.sendEventWithNameNew(event, oFF.UiEvent.ON_KEY_UP);
};
...
oFF.QuasarEngine.prototype.recursiveExtend = function(directory, element)
{
	if (oFF.notNull(element))
	{
		if (element.isStructure())
		{
			...
			var resourceRef = structure.getStringByKey(oFF.UiQuasarConstants.QSA_RESOURCE_REF);
...
oFF.QuasarProgram.prototype.resolveArgs = function()
{
	...
	{
		var normalizedFilePath = prgArgs.getStringByKey(oFF.DfProgram.PARAM_FILE);
...
oFF.QuasarModule.getInstance = function()
{
	if (oFF.isNull(oFF.QuasarModule.s_module))
	{
		...
		oFF.ProgramRegistration.setProgramFactory(new oFF.QuasarProgram());
