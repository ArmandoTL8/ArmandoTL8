    ...
    SearchableContent.prototype._collectLaunchPageTiles = function () {
        return new Promise(function (resolve, reject) {
            this._oLaunchPageService.getCatalogs().then(function (aCatalogs) {
                var aCatalogTileDeferreds = [];
                aCatalogs.forEach(function (oCatalog) {
                    aCatalogTileDeferreds.push(this._oLaunchPageService.getCatalogTiles(oCatalog));
                }.bind(this));

                var oGroupTileDeferred = this._oLaunchPageService.getGroups().then(function (aGroups) {
                    var aTiles = [];
                ...
                Promise.all([
                    Promise.all(aCatalogTileDeferreds),
                    oGroupTileDeferred
                ]).then(function (aResults) {
                    var aCatalogTileResults = aResults[0];
                    var aGroupTiles = aResults[1];
                    var aCatalogTiles = [];

                    aCatalogTileResults.forEach(function (aResult) {
                        Array.prototype.push.apply(aCatalogTiles, aResult);
                    });

                    resolve([aCatalogTiles, aGroupTiles]);
                });
            }.bind(this));
        }.bind(this));
    };
