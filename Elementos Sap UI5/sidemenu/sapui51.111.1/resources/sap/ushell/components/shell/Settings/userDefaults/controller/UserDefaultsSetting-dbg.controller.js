...
sap.ui.define([
    ...
    return Controller.extend("sap.ushell.components.shell.Settings.userDefaults.controller.UserDefaultsSetting", {
        onInit: function () {
            ...
            this.oCurrentParameters = {}; //the currently maintains parameters
            ...
            return new Promise(function (resolve, reject) {
                sap.ushell.Container.getServiceAsync("UserDefaultParameters").then(function (UserDefaultParameters) {
                    UserDefaultParameters.editorGetParameters(oSystemContext).done(function (oParameters) {
                        ...
                        this.oCurrentParameters = fnDeepExtend({}, oParameters);
                        ...
                        return this._getFormattedParameters(oParameters, this.oModel)
                            .then(function (aPreparedModel) {
                                var aGroups = this._getContent(aPreparedModel);
         ...
         * @property {ParameterModelBind} modelBind A model bindning of the parameter
         * @property {ParameterMetadata} editorMetadata A metadata of the parameter
         */

        /**
         * Format original parameters to have the same structure. Also create modelBind for the
         * @param {object} oOriginalParameters original parameters loaded from the backend
         ...
         */
        _getFormattedParameters: function (oOriginalParameters, oJsonModel) {
            var aParametersPromises = Object.keys(oOriginalParameters).map(function (sParameter, index) {
                var oParameter = fnDeepExtend({}, oOriginalParameters[sParameter]);
         ...
         *
         * @returns {Promise<ParameterModelBind>} The modal binding for the parameter
         */
        _createOdataModelBinding: function (oParameter, oJsonModel) {
            var oEditorInfo = oParameter.editorMetadata.editorInfo,
                oODataServiceData = this._getODataServiceData(oEditorInfo.odataURL, oParameter);
         ...
         */
        _getContent: function (aFormattedParameters) {
            ...
            for (var i = 0; i < aFormattedParameters.length; ++i) {
                var oProperty = aFormattedParameters[i],
                    oModelBinding = oProperty.modelBind;
         ...
         * @returns {sap.m.FlexBox} FlexBox which containts a label, an input and the extendedUsage button
         */
        ...
        saveExtendedValue: function (oControlEvent) {
            var sParameterName = oControlEvent.getSource().getModel().getProperty("/parameterName"),
                ...
                oExtendedModel = this.oModel,
                aTokens = oControlEvent.getParameters().tokens || [],
                sPathToTokens = "/" + sParameterName + "/valueObject/extendedValue/Ranges",
                aFormattedTokensData,
                oValueObject = { extendedValue: { Ranges: [] } };

            fnDeepExtend(this.oCurrentParameters[sParameterName].valueObject, oValueObject);
            ...
            aFormattedTokensData = aTokens.map(function (oToken) {
        ...
        storeChangedData: function () {
            ...
            for (var i = 0; i < aDisplayedUserDefaults.length; ++i) {
                sParameterName = aDisplayedUserDefaults[i].parameterName;
                oValueObject = this.oCurrentParameters[sParameterName].valueObject;
                oModelBinding = aDisplayedUserDefaults[i].modelBind;
                ...
                if (!this.oBlockedParameters[sParameterName]) {
                    ...
                    if (oModelBinding && oModelBinding.model) {
                        ...
                        //instead of empty string we save in the oCurrentParameters undefined
                        var sActValue = oModel.getProperty(sPropValuePath) !== "" ? oModel.getProperty(sPropValuePath) : undefined;
                        var oNewValue = {
                            value: sActValue,
                            extendedValue: oModelExtended.getProperty("/" + sParameterName + "/valueObject/extendedValue") || undefined
                        };
                        if (!fnDeepEqual(oNewValue, oldValue)) {
                            oValueObject.value = sActValue;
                            if (oNewValue.extendedValue) {
                                oValueObject.extendedValue = {};
                                fnDeepExtend(oValueObject.extendedValue, oNewValue.extendedValue);
                            }
                            this.oChangedParameters[sParameterName] = true;
                        }
                    }
        ...
        onCancel: function () {
            ...
            if (aChangedParameterNames.length > 0) {
                ...
                this.oCurrentParameters = fnDeepExtend({}, this.oOriginalParameters);
        ...
        onSave: function () {
            ...
            if (aChangedParameterNames.length === 0) {
                ...
                oPromise = sap.ushell.Container.getServiceAsync("ClientSideTargetResolution")
            ...
            return oPromise.then(this._resetSmartVariantManagement.bind(this))
        ...
        displayDiffText: function () {
            ...
            var oSmartVariantManagement = this.getView().byId("defaultSettingsVariantManagement"),
                oUserDefaultsForm = this.getView().byId("userDefaultsForm"),
                sStandardKey = oSmartVariantManagement.getStandardVariantKey(),
                sSelectionKey = oSmartVariantManagement.getSelectionKey(),
                oStandardContent, oVariantContent;

            if (sSelectionKey === sStandardKey) { return false; }

            oStandardContent = oSmartVariantManagement.getVariantContent(oUserDefaultsForm, sStandardKey);
            oVariantContent = oSmartVariantManagement.getVariantContent(oUserDefaultsForm, sSelectionKey);
