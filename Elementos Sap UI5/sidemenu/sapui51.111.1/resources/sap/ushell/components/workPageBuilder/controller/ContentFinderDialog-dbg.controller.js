...
sap.ui.define([
    ...
    return Controller.extend("sap.ushell.components.workPageBuilder.controller.ContentFinderDialog", {
        ...
        init: function () {
            ...
            this._oAppSearchModel = new JSONModel({
                data: {
                    AppSearchData: {
                        tiles: [],
                        cards: [],
            ...
            this._oAppSearchModel.setSizeLimit(Infinity);
        },
        ...
        /**
         * Called once the visualization data is loaded.
         * @param {string} sId The type of widget selected.
         * @private
         */
        _onVisualizationsLoaded: function (sId) {
            ...
            var oContentFinderAppSearchData = this._prepareAppSearchModel(oEditModel.getProperty("/data/Visualizations"), aPreselectedWidgets);
            this._oAppSearchModel.setProperty("/data/AppSearchData/tiles", oContentFinderAppSearchData.Tiles);
            this._oAppSearchModel.setProperty("/data/AppSearchData/cards", oContentFinderAppSearchData.Cards);
            this._oAppSearchModel.setProperty("/data/AppSearchData/appBoxCount", sId === "widgets-tiles"
         ...
         *
         * @param {object[]} aVisualizationsData The data coming from the content API.
         ...
         */
        _prepareAppSearchModel: function (aVisualizationsData, aPreselectedWidgets) {
            var oData = {};
            var aTilesData = aVisualizationsData.filter(function (tile) {
                return [
            ...
            oData.Tiles = aTilesData.map(function (tile, index) {
                if (!tile.Descriptor) {
            ...
            var aCardsData = aVisualizationsData.filter(function (tile) {
                return tile.Type === "sap.card";
            });

            oData.Cards = aCardsData.map(function (card, index) {
                if (!card.Descriptor) {
         ...
         * Afterwards an Event is fired to trigger a data update from WorkPage side. (visualizationFilterApplied)
        ...
        _navigateToWidgetSelectionPage: function (sWidgetType) {
            ...
            if (sWidgetType === "widgets-tiles") {
                aFilters = ["sap.ushell.StaticAppLauncher", "sap.ushell.DynamicAppLauncher"];
            ...
            var oEditModel = this.oWorkPageDataProvider.oWorkPageBuilderView.getModel();
            this._oAppSearchModel.setProperty("/data/AppSearchData", this.oContentFinderData);
            oEditModel.attachEventOnce("propertyChange", this._onVisualizationsLoaded.bind(this, sWidgetType));

            this.oWorkPageDataProvider.oWorkPageBuilderView.getController().getOwnerComponent().fireEvent("visualizationFilterApplied", {
        ...
        addWidget: function () {
            ...
            if (!this._isMyHomeWidget(sSelectedWidget) && iSelectedAppCount > 0) {
                ...
                aContentFinderSelectedTiles.forEach(function (selectedTile) {
                    var sVizSelectedItemPath = this.oWorkPageDataProvider.sUsedVizRootPath + "/" + selectedTile.vizData.Id;
                    if (!oModel.getProperty(sVizSelectedItemPath)) {
                        oModel.setProperty(sVizSelectedItemPath, selectedTile.vizData);
                    }
                }.bind(this));
        ...
        addCardSelected: function (oEvent) {
            var oSelectedCard = oEvent.getParameters().getSource().getBindingContext().getObject().vizData;
            var oModel = this.oWorkPageDataProvider.oWorkPageBuilderView.getModel();
            var sVizSelectedItemPath = this.oWorkPageDataProvider.sUsedVizRootPath + "/" + oSelectedCard.Id;
        ...
        close: function () {
            try {
                if (this.oContentFinderAppSearchComponent) {
                    this.oContentFinderAppSearchComponent.getRootControl().getController().resetFilters();
                }
            } catch (e) {
        ...
        */
        onAppBoxContainerCreated: function (oEvent) {
            this.oContentFinderAppSearchComponent = oEvent.getSource().getComponentInstance();
            this.oContentFinderAppSearchView = this.oContentFinderAppSearchComponent.getRootControl();
            ...
            if (this.oContentFinderData) {
                this.oContentFinderAppSearchView.setModel(this._oAppSearchModel);
                this.oContentFinderAppSearchView.bindObject({ path: "/data/AppSearchData" });
            }
            this.oContentFinderAppSearchComponent.attachEvent("cardSelected", this.addCardSelected.bind(this));
        },
