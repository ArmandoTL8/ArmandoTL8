    ...
    return Controller.extend("sap.ushell.components.workPageBuilder.controller.WorkPageBuilder",
        /** @lends sap.ushell.components.workPageBuilder.controller.WorkPageBuilder.prototype */ {
            ...
            setVisualizationData: function (oVizNodes) {
                var aVisualizations = ObjectPath.get("Visualizations.nodes", oVizNodes);
                this.oModel.setProperty("/data/Visualizations", aVisualizations);
                this.oModel.firePropertyChange({ type: "Visualizations" });
            },
            ...
            tileEditMode: function (bEditMode, aWidgets) {
                var oModel = this.getView().getModel();
                var oUsedViz;
                return bEditMode && !!aWidgets && aWidgets.every(function (oWidget) {
                    oUsedViz = oModel.getProperty("/data/UsedVisualizations/" + ObjectPath.get("Visualization.Id", oWidget));
                    return ["sap.ushell.DynamicAppLauncher", "sap.ushell.StaticAppLauncher"].indexOf(ObjectPath.get("Type", oUsedViz)) > -1;
                });
            },
