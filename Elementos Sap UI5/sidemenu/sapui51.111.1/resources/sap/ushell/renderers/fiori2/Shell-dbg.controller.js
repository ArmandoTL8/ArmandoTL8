...
sap.ui.define([
    ...
    return Controller.extend("sap.ushell.renderers.fiori2.Shell", {
        ...
        _doHashChange: function (oShellController, sShellHash, sAppPart, sOldShellHash, sOldAppPart, oParseError) {
            ...
            var oInMemoryApplicationInstance, iOriginalHistoryLength, sFixedShellHash, sOldFixedShellHash, oDeferred;
            ...
            sOldFixedShellHash = oShellController.fixShellHash(sOldShellHash);
        ...
        _openAppViaNWBC: function (oResolvedHashFragment) {
            ...
            return new Promise(function (fnResolve) {
                ...
                oPromiseParams = that._appendUserIdToUrl("sap-user", oResolvedHashFragment.url).then(function (sUrlWithSapUser) {
                    ...
                    return new Promise(function (fnResolveParams) {
                        try {
                            var sAppType = oResolvedHashFragment.applicationType,
                                sFrameworkId = (oResolvedHashFragment.appCapabilities && oResolvedHashFragment.appCapabilities.appFrameworkId),
                                oFLPParams = {},
                                aInfoArray = [],
                                aKeysArray,
                                fnPreparePostBodyParams = function () {
                                ...
                                getParamKeys = function getParamKeys (sUrl, aInfoArray) {
                                    var aAppStateKeysArray = [],
                                        aParams,
                                        fnAddKey = function (sName, sDataName) {
                                            if (sUrl.indexOf(sName + "=") > 0) {
                                                aParams = new RegExp("(?:" + sName + "=)([^&/]+)").exec(sUrl);
        ...
        _handleEmbeddedNavMode: function (sFixedShellHash, oParsedShellHash, oMetadata, oResolvedHashFragment, sOldFixedShellHash) {
            ...
            oHandlerPromise = AppLifeCycleAI.handleControl(
                sIntent,
                sAppId,
                oParsedShellHash,
        ...
        getWrappedApplicationWithMoreStrictnessInIntention: function (sIntent, oMetadata, oShellHash, oResolvedNavigationTarget, sAppId, bFullWidth, sFixedShellHash) {
            ...
            oAppContainer = AppLifeCycleAI.getAppContainer(sAppId, oResolvedNavigationTarget, this._isColdStart(), oShellHash, sFixedShellHash);
            ...
            sAppType = oAppContainer.getApplicationType();
            sFrameworkId = oAppContainer.getFrameworkId();
