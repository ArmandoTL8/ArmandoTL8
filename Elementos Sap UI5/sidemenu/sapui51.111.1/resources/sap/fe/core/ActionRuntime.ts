import type { EntitySet, PrimitiveType } from "@sap-ux/vocabularies-types";
...
const ActionRuntime = {
				...
				if (oTable && oTable.isA<Table>("sap.ui.mdc.Table")) {
					...
					if (oRowBinding == oListBinding) {
						ActionRuntime.setActionEnablement(
							...
							oTable.getSelectedContexts() as V4Context[],
							"table"
						);
					}
				}
	...
	updateEditButtonVisibilityAndEnablement(oView: View) {
		...
		if ((iViewLevel as number) > 1) {
			const oContext = oView.getBindingContext() as V4Context;
			...
			if (sPath) {
				const oRootContext = oView.getModel().bindContext(sPath).getBoundContext() as V4Context;
	...
	_setActionEnablementForTable: function (
		oSelectedContext: V4Context | undefined,
		oInternalModelContext: InternalModelContext,
		sAction: string,
		sProperty: string,
