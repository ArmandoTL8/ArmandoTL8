...
import type { SideEffectsTargetEntityType } from "../../services/SideEffectsServiceFactory";
...
async function editDocumentInStickySession(context: Context, appComponent: AppComponent): Promise<Context> {
	...
	const resourceBundle = await (appComponent.getModel("sap.fe.i18n") as ResourceModel).getResourceBundle();
...
async function activateDocument(context: Context, appComponent: AppComponent) {
	...
	try {
		...
		const messagesPath = metaModel.getObject(`${metaPath}/@${CommonAnnotationTerms.Messages}/$Path`);

		if (messagesPath) {
			try {
				await appComponent
					.getSideEffectsService()
					.requestSideEffects([{ $PropertyPath: messagesPath }] as SideEffectsTargetEntityType[], context);
			} catch (error: unknown) {
		...
		return MessageBox.warning(warningMsg, {
			...
			onClose: function (actionText: string) {
				if (actionText === confirmButtonTxt) {
					const internalModel = view && (view.getModel("internal") as JSONModel);
