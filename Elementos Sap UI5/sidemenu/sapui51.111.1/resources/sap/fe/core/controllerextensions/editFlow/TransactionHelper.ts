...
import type { ODataListBinding } from "types/extension_types";
...
class TransactionHelper {
		...
		try {
			if (sNewAction) {
				...
				const aNonComputedVisibleKeyFields = bIsNewPageCreation
					? CommonUtils.getNonComputedVisibleFields(oMetaModel, sMetaPath, currentView)
					: [];
				...
				if (mParameters) {
					if (!mParameters.numberOfSelectedContexts) {
						...
						if (mParameters.title) {
							...
							nonTableTxt = CommonUtils.getTranslatedText(
	...
	async saveDocument(
		...
		try {
			...
			if (!(messagesReceived.length === 1 && messagesReceived[0].type === coreLibrary.MessageType.Success)) {
				...
				MessageToast.show(
					isNewObject
						? CommonUtils.getTranslatedText("C_TRANSACTION_HELPER_OBJECT_CREATED", resourceBundle)
						: CommonUtils.getTranslatedText("C_TRANSACTION_HELPER_OBJECT_SAVED", resourceBundle)
				);
			}
	...
	async callAction(
		...
		const displayUnapplicableContextsDialog = (): Promise<ODataV4Context[] | void> => {
	 ...
	 * @final
	 */
	_confirmDiscard(cancelButton: Button, isModified: boolean, resourceBundle: ResourceBundle): Promise<void> {
