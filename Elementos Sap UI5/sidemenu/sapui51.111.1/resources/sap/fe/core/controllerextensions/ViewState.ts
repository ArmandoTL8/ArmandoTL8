...
import type { MetaModelNavProperty, VariantManagement } from "types/extension_types";
...
const _mControlStateHandlerMap: Record<string, any> = {
	...
	"sap.ui.mdc.FilterBar": {
		...
		apply: async function (filterBar: FilterBar, controlState: ControlState) {
			try {
				if (controlState) {
					if (controlState?.initialState && this.controlsVariantIdUnavailable.indexOf(filterBar.getId()) === -1) {
		...
		apply: async function (table: Table, controlState: ControlState) {
			try {
				if (controlState) {
					...
					if (controlState?.initialState && this.controlsVariantIdUnavailable.indexOf(table.getId()) === -1) {
						if (!controlState.initialState?.supplementaryConfig) {
