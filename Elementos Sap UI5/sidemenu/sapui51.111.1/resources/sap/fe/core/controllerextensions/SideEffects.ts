...
class SideEffectsControllerExtension extends ControllerExtension {
	...
	getFieldSideEffectsMap(field: Control): FieldSideEffectDictionary {
		...
		sideEffectsMap = this.getSideEffectsMapForFieldGroups(fieldGroupIds, field) as FieldSideEffectDictionary;
		...
		if (viewEntitySetSetName && viewEntitySet) {
			const viewEntityType = viewEntitySet.entityType.fullyQualifiedName,
				...
				context = this.getContextForSideEffects(field.getBindingContext(), viewEntityType) as V4Context | undefined;
	...
	getSideEffectsMapForFieldGroups(
		...
		fieldGroupIds.forEach((fieldGroupId) => {
			...
			const oContext = field
				? (this.getContextForSideEffects(field.getBindingContext(), sideEffectEntityType) as V4Context)
				: undefined;
	...
	@publicExtension()
	@finalExtension()
	isFieldGroupValid(fieldGroupId: string, context: V4Context): boolean {
	 ...
	 * @returns Index
	 */
	private _getFieldGroupIndex(fieldGroupId: string, context: V4Context): string {
			...
			for (const context of [field.getBindingContext(), this._view.getBindingContext()]) {
				if (context) {
					...
					for (const failedSideEffect of failedSideEffects) {
						failedSideEffectsPromises.push(this.requestSideEffects(failedSideEffect, context as V4Context));
					}
				}
			}
