...
type TargetTableInfoType = {
	...
	oTableRowContext: Context | undefined;
...
export type messageHandlingType = {
	...
	determineColumnInfo: (oColFromTableSettings: any, oResourceBundle: ResourceBundle) => ColumnInfoType;
...
function getTableColumnDataAndSetSubtile(
	...
	const oResourceBundle = Core.getLibraryResourceBundle("sap.fe.core") as any;
...
function getMessageSubtitle(
	...
	if (bIsCreationRow) {
		...
		if (oTableRowBindingContexts.length > 0) {
			...
			const oColumnInfo: ColumnInfoType = messageHandling.determineColumnInfo(oColFromTableSettings, oResourceBundle);
 ...
 * @returns Column data.
 */
function determineColumnInfo(oColFromTableSettings: any, oResourceBundle: ResourceBundle): ColumnInfoType {
	...
	if (oColFromTableSettings) {
		...
		if (oColFromTableSettings.availability === "Hidden") {
			...
			oColumnInfo.sColumnValue = `${oResourceBundle.getText("T_MESSAGE_GROUP_DESCRIPTION_TABLE_COLUMN")} (${oResourceBundle.getText(
...
function createSectionGroupName(
	...
	return (
		...
		(oTargetTableInfo ? `, ${oResourceBundle.getText("T_MESSAGE_GROUP_TITLE_TABLE_DENOMINATOR")}: ${oTargetTableInfo.tableHeader}` : "")
	);
}
