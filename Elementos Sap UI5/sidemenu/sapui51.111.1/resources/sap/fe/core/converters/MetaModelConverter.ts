	...
	for (const entitySet of result.schema.entitySets) {
		...
		if (navPropertyBindings) {
			for (const navPropName of Object.keys(navPropertyBindings)) {
				const targetEntitySet = result.schema.entitySets.find(
					(entitySetName) => entitySetName.name === navPropertyBindings[navPropName]
				);
				if (targetEntitySet) {
...
function parseAnnotations(annotations: Record<string, any>, capabilities: EnvironmentCapabilities) {
	...
	return Object.keys(annotationLists)
		.sort((a, b) => a.length - b.length)
		.map((annotationName) => annotationLists[annotationName]);
}
...
export function getInvolvedDataModelObjectFromPath(
	...
	const dataModelObjects = metaModelContext.visitedObjects.filter(
		(visitedObject: any) =>
			visitedObject?.hasOwnProperty("_type") && visitedObject._type !== "EntityType" && visitedObject._type !== "EntityContainer"
	);
	if (
