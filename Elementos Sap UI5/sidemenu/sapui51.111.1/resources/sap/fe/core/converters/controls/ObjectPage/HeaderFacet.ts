...
function createReferenceHeaderFacet(
	...
	if (facetDefinition.$Type === UIAnnotationTypes.ReferenceFacet && !(facetDefinition.annotations?.UI?.Hidden?.valueOf() === true)) {
		...
		if (
			facetDefinition.Target?.$target?.term === UIAnnotationTerms.Chart &&
			isAnnotationFieldStaticallyHidden(facetDefinition as any)
		) {
			return undefined;
		} else {
			return {
...
function getDataPointData(facetDefinition: FacetTypes, converterContext: ConverterContext): HeaderDataPointData {
	...
	if (facetDefinition.$Type === UIAnnotationTypes.ReferenceFacet && !isAnnotationFieldStaticallyHidden(facetDefinition as any)) {
