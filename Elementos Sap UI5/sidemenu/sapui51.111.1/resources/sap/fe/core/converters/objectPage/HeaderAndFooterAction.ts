...
export function getHeaderDefaultActions(converterContext: ConverterContext): BaseAction[] {
	...
	const entitySet = converterContext.getEntitySet(),
		...
		updateHidden = rootEntitySet && !ModelHelper.isSingleton(rootEntitySet) && rootUpdateHidden?.valueOf(),
	...
	if (!ModelHelper.isSingleton(entitySet) && draftRoot?.EditAction && updateHidden !== true) {
