...
export function createSubSections(
	...
	return facetsToCreate.map((facet) =>
		createSubSection(facet, facetsToCreate, converterContext, 0, !(facet as any)?.Facets?.length, isHeaderSection)
	);
}
...
export function createSubSection(
	...
	const oHiddenAnnotation: any = facetDefinition.annotations?.UI?.Hidden;
	const isVisible = compileExpression(not(equal(true, getExpressionFromAnnotation(oHiddenAnnotation))));
	const isDynamicExpression =
		isVisible && typeof isVisible === "string" && isVisible.indexOf("{=") === 0 && oHiddenAnnotation?.type !== "Path";
	const isVisibleDynamicExpression =
		isVisible && isDynamicExpression ? isVisible.substring(isVisible.indexOf("{=") + 2, isVisible.lastIndexOf("}")) : false;
			...
			if (!facetDefinition.Target.$target) {
				...
				switch (facetDefinition.Target.$target.term) {
					...
					case UIAnnotationTerms.SelectionPresentationVariant:
						...
						const controlTitle =
							(presentation.visualizations[0] as any)?.annotation?.title || (presentation.visualizations[0] as any)?.title;
						...
						const dataVisualizationSubSection: DataVisualizationSubSection = {
							...
							showTitle: showTitle,
							titleVisible: isDynamicExpression
								? `{= (${isVisibleDynamicExpression}) && ('${title}' !=='undefined') && (${showTitle} ? true : false) }`
								: titleVisibleExpression
						};
...
export function getTitleVisibility(
	...
	return compileExpression(
		or(not(hasSingleContent), notEqual(resolveBindingString(controlTitle), resolveBindingString(subSectionTitle)))
	);
}
