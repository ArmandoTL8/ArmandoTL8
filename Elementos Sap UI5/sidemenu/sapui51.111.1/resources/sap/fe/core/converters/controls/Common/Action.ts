...
import { DataFieldForActionTypes, UIAnnotationTypes } from "@sap-ux/vocabularies-types/vocabularies/UI";
...
import type { MetaModelType } from "types/extension_types";
...
function mapActionByKey(
	...
	const resultAction = { ...(annotationAction ?? manifestAction) };
	...
	if (annotationAction) {
		...
		for (const prop in manifestAction || {}) {
			if (!(annotationAction as any)[prop] && prop !== "menu") {
				(resultAction as any)[prop] = (manifestAction as any)[prop];
			}
		}
	}
...
function mapMenuDefaultAction(
	...
	if (action.command) {
		(commandActions as any)[action.key] = action;
	}
}
...
function mapMenuItems(
	...
	for (const menuItemKey of menuAction.menu ?? []) {
		const { action, canBeMenuItem } = mapActionByKey(manifestActions, annotationActions, hiddenActions, menuItemKey);
		...
		if (action.command) {
			(commandActions as any)[menuItemKey] = action;
		}
	}
...
function getManifestActionBooleanPropertyWithFormatter(
	...
	const resolvedBinding = resolveBindingString(propertyValue as string, "boolean");
	let result: any;
	if (isConstant(resolvedBinding) && resolvedBinding.value === undefined) {
		...
		result = formatResult(
			[pathInModel("/", "$view"), methodPath, pathInModel("selectedContexts", "internal")],
			fpmFormatter.customBooleanPropertyCheck as any,
...
export const removeDuplicateActions = (actions: BaseAction[]): BaseAction[] => {
	let oMenuItemKeys: { [key: string]: any } = {};
	actions.forEach((action) => {
		if (action?.menu?.length) {
			oMenuItemKeys = action.menu.reduce((item, { key }: any) => {
				if (key && !item[key]) {
					item[key] = true;
				}
				return item;
