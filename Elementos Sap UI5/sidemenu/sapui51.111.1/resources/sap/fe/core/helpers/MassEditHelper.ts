...
import type { ODataSideEffectsType, SideEffectsTargetEntityType, SideEffectsType } from "sap/fe/core/services/SideEffectsServiceFactory";
...
import SideEffectsHelper from "./SideEffectsHelper";
...
const MassEditHelper = {
	 ...
	 */
	getSideEffectDataForKey: function (oEntitySetContext: any, aSideEffects: any, oController: PageController, aResults: any) {
	 ...
	 */
	fnGetTargetsForMassEdit: function (sideEffectsData: any, oEntitySetContext: any, sEntityType: any, oMetaModel: any) {
	...
	checkIfEntityExistsAsTargetEntity: (
		...
		const sideEffectsWithCurrentEntityAsTarget = sideEffectsOnOtherEntity.filter((obj: MassEditFieldSideEffectPropertyType) => {
			const targetEntitiesArray: SideEffectsTargetEntityType[] | undefined = obj.sideEffects.TargetEntities;
			return targetEntitiesArray?.filter(
				(innerObj: SideEffectsTargetEntityType) => innerObj["$NavigationPropertyPath"] === entitySetName
			).length
				? obj
				: false;
		});
			...
			for (const [index, data] of sideEffectsDataForField.entries()) {
				...
				for (const aSideEffect of sideEffectsForCurrentContext) {
					if (!mVisitedSideEffects[oContext].hasOwnProperty(aSideEffect.fullyQualifiedName)) {
						...
						const fnGetImmediateTargetsAndActions = async function (mSideEffect: SideEffectsType) {
							...
							const actionName: string | undefined = (mSideEffect as ODataSideEffectsType).TriggerAction;
