...
const PasteHelper = {
	...
	pasteData: function (rawData: any, table: any, controller: PageController): Promise<any> {
		const oInternalEditFlow = (controller as any)._editFlow;
		...
		return this.parsePastedData(rawData, table)
			.then((aParsedData) => {
				...
				return Promise.all(
					aData.map((mData) =>
						oInternalEditFlow.getTransactionHelper().validateDocument(
							table.getBindingContext(),
							{
								data: mData,
			...
			.then((aValidatedData) => {
				return aValidatedData.length > 0
					? oInternalEditFlow.createMultipleDocuments(
							table.getRowBinding(),
							aValidatedData,
