	...
	function getMethods(oTemplateContract, oNavigationControllerProxy) {
		...
		function fnInitSession(bIsFirst){
			...
			if (bIsRestoring){
				...
				var oInitialNavigationPromise = oCrossAppNavigator.isInitialNavigationAsync().then(function(bIsInitialNavigation){
		...
		function fnWillNavigationToNodeShowPlaceholder(oTreeNode){
			return oCurrentHash.componentsDisplayed[oTreeNode.sRouteName] !== 1 && (oTreeNode.level > 0 || !oTreeNode.componentId) && (oTreeNode.behaviour && oTreeNode.behaviour.getPlaceholderInfo());
		}
		...
		function fnNavigateForNonDraftCreate(sEntitySet, vPredefinedValues, oPredefinedContext, mAppStates){
			...
			if (vPredefinedValues){
				oPredefinedKeyPromise = oCrossAppNavigator.createEmptyAppStateAsync(oTemplateContract.oAppComponent).then(function(oAppState){
