	...
	var oMethods = {
		getMethods: function (oViewProxy, oTemplateUtils, oController) {
						...
						if (IsActiveEntity === false) { // We have not negated it because we wanted to differentiate between false and undefined
							...
							return oSiblingContextPromise.then(function (oSiblingContext) {
								return ShareUtils.getCurrentUrl().then(function (sCurrentUrl) {
									var sPath = URLParser ? sCurrentUrl.split(URLParser.getHash(sCurrentUrl))[0] + URLParser.getShellHash(sCurrentUrl) + "&" + oSiblingContext.sDeepPath : sCurrentUrl;
									var sUrl = URLParser ? "#" + URLParser.getShellHash(document.URL) + "&" + oSiblingContext.sDeepPath : oSiblingContext.sDeepPath;
									return {
										serviceUrl: "",
			...
			*/
					...
					if (controlHelper.isSmartTable(oControl)){
						...
						return oFirstEditableCellPromise.then(function(oEditableCell){
							...
							if (oTemplateUtils.oComponentUtils.isDraftEnabled()) {
								...
								var oToolbar = oControl.getCustomToolbar() || oControl.getToolbar();
			...
			//function hide heeader container of Object page
			function fnHideHeaderContainer() {
				var oTemplatePrivateModel = oTemplateUtils.oComponentUtils.getTemplatePrivateModel();
				oTemplatePrivateModel.setProperty("/objectPage/showHeaderContent", false);
			}

			function fnSetObjectPageHeaderContent(oObjectPage) {
				...
				oTemplatePrivateModel.setProperty("/objectPage/showHeaderContent", true);
				var aHeaderContent = oObjectPage.getHeaderContent();
				var sHeaderType = oController.getOwnerComponent().getAppComponent().getObjectPageHeaderType();
				var bShowHeaderAsCarouselOnDesktop = oController.getOwnerComponent().getShowHeaderAsCarouselOnDesktop();
				if (sHeaderType === "Dynamic") {
					if (Device.system.desktop) {
						if (bShowHeaderAsCarouselOnDesktop && aHeaderContent && aHeaderContent[0] && !aHeaderContent[0].getContent().length) {
							fnHideHeaderContainer();
						} else if (!bShowHeaderAsCarouselOnDesktop && aHeaderContent && aHeaderContent[0] && !aHeaderContent[0].getItems().length) {
							fnHideHeaderContainer();
						}
					} else if (aHeaderContent && aHeaderContent[0] && !aHeaderContent[0].getContent().length) {
						fnHideHeaderContainer();
					}
				} else if (sHeaderType === "Static" && aHeaderContent.length === 0) {
					fnHideHeaderContainer();
				}
			}
			...
			var oControllerImplementation = {
				onInit: function () {
					...
					fnSetObjectPageHeaderContent(oObjectPage);
					...
					oState.oInlineCreationRowsHelper = new InlineCreationRowsHelper(oObjectPage, oTemplateUtils, fnGetSmartTableCreationMode);
