					...
					oModel.createBindingContext(sBindingPath, null, null, function(oBindingContext){
						if (oDraftContext.isDraftEnabled(sEntitySet)) {
							...
							if (true || !oDraftContext.hasPreserveChanges(oBindingContext)) {
								fnReadDraftAdminstrativeData(oModel, sBindingPath, oTemplateContract.oBusyHelper).then(function(oResponse) {
									if (!oResponse.DraftAdministrativeData || oResponse.DraftAdministrativeData.DraftIsCreatedByMe) {
										// no or own draft
										fnEditEntity(oBindingContext);
									} else if (oResponse.DraftAdministrativeData.InProcessByUser) { // locked
		...
		*/
		...
		function activateDraftEntity(oCreateDialogContext, oCreateWithDialogFilter, oBusyHelper, oServices, oController, oComponentUtils) {
			...
			var oRet = new Promise(function(resolve, reject) {
				...
				var aCurrentStateMessages; // This varaible will be filled after a failure of the first call of function fnActivate (see below)
				...
				var fnActivate = function(bForce){
					...
					// In a potentail second call of this function this information is already available in aCurrentStateMessages.
					...
					oActivationPromise.catch(function(oError){
						...
						// Note that in case 2 and possibly also in case 1 we need to supress the generic error message (like "An exception has occured") which is coming as a transient message to
						...
						// - all state messages
						// - all transient messages of severity warning
						// The standard transient message dialog will be supressed in this case. In particular the generic error message named above will not be shown.
						// If case 1 is not applicable case 2 is considered applicable if at least one state message of severity error is available.
						// Also in this case the standard transient message dialog will be supressed.
						...
						if (bIsWarning){
							...
							var oParameters = {
								messagesForUserDecison: aCurrentStateMessages.concat(aTransientWarnings)
							};
							var oCRUDActionHandler = oComponentUtils.getCRUDActionHandler();
							oCRUDActionHandler.handleCRUDScenario(4, fnActivate.bind(null, true), reject, "Activate", oParameters);
							return;
						}
						reject();
						if (bHasStateError){
