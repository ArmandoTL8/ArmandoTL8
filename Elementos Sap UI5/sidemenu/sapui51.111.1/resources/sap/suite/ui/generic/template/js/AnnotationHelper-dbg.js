...
*/
		...
		getShareButtonVisibility: function(bIsFlexibleColumnLayout, sTreeNodeLevel) {
			if (bIsFlexibleColumnLayout) {
				return '{= !(${_templPrivGlobal>/generic/isTeamsModeActive} || ${_templPrivGlobal>/generic/FCL/highestViewLevel} !== ' + sTreeNodeLevel + ' || ${ui>/createMode}) }';
			} else {
				return '{= !(${_templPrivGlobal>/generic/isTeamsModeActive} || ${ui>/createMode})}';
			}
		},

		getShareOptionVisibility: function(bFlexibleColumnLayout) {
            if (bFlexibleColumnLayout) {
                return '{= !(${_templPrivGlobal>/generic/isTeamsModeActive} || ${_templPrivGlobal>/generic/FCL/highestViewLevel})}';
            } else {
                return '{= !${_templPrivGlobal>/generic/isTeamsModeActive}}';
            }
        },
		 ...
		 * - Expression binding
		 */
		isRelatedEntityCreatable: function (oInterface, oSourceEntitySet, oRelatedEntitySet, aSubPages, oFacet, oSections, bIsDraftEnabled, oCreateWithParameterDialog, sTableType) {
			...
			if (bHasSubObjectPage || bIsInlineCreationMode || oCreateWithParameterDialog) {
				...
				var vCreatablePath = oAnnotationHelper.getCreatablePath(oInterface, oSourceEntitySet, oRelatedEntitySet);
				...
				if (!vCreatablePath) {
					...
					aConditions.push(vCreatablePath);
				}
		...
		buildCreatablePathExpression: function (oInterface, oSourceEntitySet, oRelatedEntitySet) {
			var vCreatablePath = oAnnotationHelper.getCreatablePath(oInterface, oSourceEntitySet, oRelatedEntitySet);
			if (typeof vCreatablePath === "string") {
	...
	oAnnotationHelper.buildCreatablePathExpression.requiresIContext = true;
