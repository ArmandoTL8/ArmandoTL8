	...
	"sap/suite/ui/generic/template/lib/TemplateComponent"
		...
		function fnNavigateFromMessageTitleEvent(oTemplateUtils, oMessageSource, oComponent, bPageNavigationAllowed, sBindingPathOfSource) {
			...
			oAppNavigationPromise.then(function(bAppNavigationStarted){
				...
				oTemplateUtils.oServices.oApplication.prepareForControlNavigation(oComponent, sId).then(function(){
					var oTargetControl = controlHelper.byId(sId);
					var oFocussablePromise = controlHelper.isSmartField(oTargetControl) ? controlHelper.getSmartFieldIsFocussableForInputPromise(oTargetControl) : Promise.resolve(true);
					oFocussablePromise.then(function(bIsFocussable){
						controlHelper.focusUI5Control(bIsFocussable ? oTargetControl : oTargetControl.getParent(), oFocusInfo);
					});
				});				
			});
		}
		...
		*/
		function fnShowSuccessMessageIfRequired(sFallbackMessageText, oServices){
			...
			var bMessageAvailable = aMessages.some(function(oMessage) {
							return oMessage.persistent && oServices.oApplication.isTransientMessageNoCustomMessage(oMessage);
						});
		...
		function fnPrepareForMessageHandling(aMessages, oTemplateUtils){
			...
			return bNeedsPreparation ? oTemplateUtils.oComponentUtils.prepareForMessageHandling(aMessages) : Promise.resolve();
		}
