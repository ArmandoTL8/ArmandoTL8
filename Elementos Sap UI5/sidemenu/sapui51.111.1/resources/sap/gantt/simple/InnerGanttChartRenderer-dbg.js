...
sap.ui.define([
		...
		if (bToolbarSettingsItemChanged) {
			...
			if (aRows.length > 0){
				...
				aVisibleExpandedRows.forEach(function(i){
					...
					if (aExpandedData){
						var aExpandScheme, sShapeScheme = aExpandedData[(aExpandedData.length) - 1] && aExpandedData[(aExpandedData.length) - 1].scheme, oMain;
						aExpandScheme = oGantt.getShapeSchemes().filter(function(oScheme){
							return oScheme.getKey() === sShapeScheme;
						});
						if (aExpandScheme[0] !== undefined || (oGantt.getEnablePseudoShapesDisplay() && aExpandedData[(aExpandedData.length) - 1].expandSchemeShape)){
							oGantt._oExpandModel.isTableRowHeightNeedChange(true, oGantt.getTable(), [iExpandedIndex], oPrimaryScheme, aExpandScheme[0]);
	...
	InnerGanttChartRenderer.renderGanttChart = function(oRm, oGantt) {
		var _oRb = Core.getLibraryResourceBundle("sap.gantt");
		...
		var oLocale = oGantt.getLocale() || oGantt.getAxisTime().getLocale();
		if (oLocale) {
			oGantt.setLocale(oLocale, true);
			oGantt.getAxisTime().setLocale(oLocale, true);
		}
		oRm.close("div");
	};
	...
	InnerGanttChartRenderer.renderRowBackgrounds = function(oRm, oGantt, aRowStates) {
		...
		var visibleRowCount = oGantt.getTable().getVisibleRowCount();
		...
		if (oGantt.getDisplayType() === library.simple.GanttChartWithTableDisplayType.Chart) {
			document.getElementById(oGantt.getId() + "-sapGanttBackgroundTableContent").style.height = nHeightOfPreviousRows + "px";
			var sapUiTableCnt =  document.getElementById(oGantt.getId() + "-sapUiTableCnt");
			var sapUiTableColHdrCnt = document.getElementsByClassName("sapUiTableColHdrCnt");
			var iTop =  sapUiTableCnt && sapUiTableCnt.offsetTop;
			var iHeaderHeight = sapUiTableColHdrCnt && sapUiTableColHdrCnt[0].getBoundingClientRect().height;
			document.getElementById(oGantt.getId() + "-ganttHeader").style.height = iTop + iHeaderHeight + "px";
		}
	};
