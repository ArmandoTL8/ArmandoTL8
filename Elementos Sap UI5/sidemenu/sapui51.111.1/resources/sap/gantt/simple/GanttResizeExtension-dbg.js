...
sap.ui.define([
	...
	GanttResizeExtension.prototype.getSelectionInlineStyle = function(oElement) {
		...
		var mStyles = {
			fill: Parameters.get(shapeColor) ? Parameters.get(shapeColor) : shapeColor,
			stroke: Parameters.get(color) ? Parameters.get(color) : color,
	...
	GanttResizeExtension.prototype.endResizing = function (event) {
		...
		if (oShape) {
			...
			if (oGanttChart.getEnablePseudoShapesDisplay()){
	...
	GanttResizeExtension.prototype.getConnectSquaresData = function(bShowConnectIndicator, oElement) {
		...
		var iLeftX = iX - iMargin - iRectWidth - 2 + mBias.x,
			iLeftY = bIsTransformed ?  iY + (iHeight * heightFactor) - (iRectWidth / 2) : iY + (iHeight * heightFactor) - (iRectWidth / 2) + mBias.y;
	...
	GanttResizeExtension.prototype.getResizeRectCoverAttrs = function(oElement, bShape) {
		...
		var sShapeColor = Parameters.get(shapeColor) ? Parameters.get(shapeColor) : shapeColor;
		var mCommonProp = {
			stroke: "none",
			"stroke-width": 0,
			fill: bShapeSelectedFill ? null : sShapeColor,
			"class": "resizeCover",
	...
	GanttResizeExtension.prototype.getOutlineLines = function(oElement, bShape) {
		...
		var mCommonProp = {
			...
			fill: Parameters.get(color) ? Parameters.get(shapeColor) : shapeColor,
