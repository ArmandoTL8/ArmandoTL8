...
sap.ui.define([
	...
	BaseShape.prototype.setTime = function (oDate, bSuppressInvalidate) {
		if (oDate) {
			this.setProperty("time", GanttUtils.getFormatedDateByTimeZone(oDate), bSuppressInvalidate);
		}
	};

	BaseShape.prototype.setEndTime = function (oDate, bSuppressInvalidate) {
		if (oDate) {
			this.setProperty("endTime", GanttUtils.getFormatedDateByTimeZone(oDate), bSuppressInvalidate);
		}
	};
	...
	BaseShape.prototype.onclick = function(oEvent) {
		var fnClickEvent = function () {
			...
			if (this.bPreventSingleClick === false) {
				...
				if (!oGanttChart.fireShapePress({popoverOffsetX: this._getPopoverOffsetX(oEvent), rowSettings: this.getParentRowSettings(), shape: this, ctrlOrMeta: oEvent.ctrlKey || oEvent.metaKey})) {
					return;
				}
