	...
	var RenderUtils = {
		...
		renderElementTitle : function(oRm, oElement, fTitleCreator) {
			...
			if (sTitle) {
				...
				var bPseudoShape = oGantt && oGantt.getEnablePseudoShapesDisplay() && oParent &&  oParent.isPseudoShape;
				if (bPseudoShape){
		...
		_calculateX : function(oElement, alignment, iX, width, iCornerPaddingPixel, iTruncatedTextWidth, mTextSettings, iIconHeight, bPseudoShape){
			var bRTL = Core.getConfiguration().getRTL(), iNonRTL, iRTL;
			...
			aShapesInRow.forEach(function(aShapes, iIndex){
				Array.isArray(aShapes) && aShapes.forEach(function(oShape){
					if (oGantt.isShapeVisible(oShape) && !oShape.isPartOfExpandedPseudoShape) {
						if (oGantt.getEnablePseudoShapesDisplay()){
							var isRowExpanded;
						...
						RenderUtils.renderMainRowShape(oRm, oShape, {
							...
							rowUid: sRowUid,
							rowExpanded:oGantt.getEnablePseudoShapesDisplay() ? isRowExpanded : bHasExpandShape,
		...
		renderMainRowShape : function(oRm, oShape, mOption, bShowParentRow) {
			...
			var oShapeRow = oShape.getParent().getParent(),
			iShapeRowIndex = oShapeRow.getIndex(),
			bIncludePseudoShape = oGantt.oOverlapShapeIds && oGantt.oOverlapShapeIds[iShapeRowIndex] && oGantt.oOverlapShapeIds[iShapeRowIndex].length > 0;
			if ((Object.keys(mOption.expandModel.mExpanded).indexOf(mOption.rowUid) == -1  && !mOption.rowExpanded) || bShowParentRow || (oGantt._aExpandedIndices.indexOf(iShapeRowIndex) > -1 && (bIncludePseudoShape))){
					oShape.renderElement(oRm, oShape, null);
		...
		setSpecialProperties: function(oShape, mOption) {
			var oExpandModel = mOption.expandModel,
				...
				oGantt = oTable.getParent(),
				iShapeRowIndex = oShape.getParent().getParent().getIndex();
		...
		renderExpandShapesIfNecessary : function(oRm, oMainShape, mOption, bShowParentRow) {
			...
			if (oGantt.getUseParentShapeOnExpand()) {
				//experimental code
				var aExpandableShapes = oMainShape.getParent().getAllExpandableShapes(oGantt);
