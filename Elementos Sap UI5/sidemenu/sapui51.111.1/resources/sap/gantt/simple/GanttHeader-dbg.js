...
sap.ui.define([
...
], function (Element, Core, Device, RenderUtils, Utility, Format, Parameters, BaseRectangle, AdhocLineRenderer, GanttUtils, DeltaLineRenderer, BaseLine, GanttExtension) {
	"use strict";
	...
	GanttHeader.prototype.renderElement = function() {
		...
		if (oGantt.getEnableChartOverflowToolbar()) {
			...
			var oGanttOverflowToolbarDom = Core.byId(sGanttId + "-ganttHeaderOverflowToolbar").getDomRef();

			if (oTableOverflowToolbar[0]) {
				var oTableOverflowToolbarDom = Core.byId(oTableOverflowToolbar[0].sId).getDomRef();
