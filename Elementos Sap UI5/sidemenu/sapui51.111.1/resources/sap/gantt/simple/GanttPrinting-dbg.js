...
sap.ui.define([
	...
	function (
		 ...
		 * @experimental
		 */
		GanttPrinting.prototype.openPdfPreview = function () {
			if (!this.getGanttChart() || !this._getOriginalGanttChart().isA("sap.gantt.simple.GanttChartWithTable")) {
				throw new Error("Association 'ganttChart' of type 'sap.gantt.simple.GanttChartWithTable' is not set");
			}
			this._oDomToPrint = [];
		...
		GanttPrinting.prototype._drawClonedGantt = function (bPdfPrint) {
			...
			if (!bPdfPrint && ((this._oClonedGanttDiv.offsetWidth * this.canvasScale) > MAXCANVASSIZE || (this._oClonedGanttDiv.offsetHeight * this.canvasScale) > MAXCANVASSIZE)) {
				this._oClonedGanttDiv.style.height = (this._oClonedGanttDiv.offsetHeight * this.canvasScale) + "px";
				this._oClonedGanttDiv.style.width = (this._oClonedGanttDiv.offsetWidth * this.canvasScale) + "px";
		...
		GanttPrinting.prototype._addBackOriginalGanttRowBinding = function (oClonedGantt, oOriginalGantt) {
			...
			if (oClonedGantt.getEnablePseudoShapesDisplay()){
		...
		GanttPrinting.prototype._generateSvgToPdf = function () {
			...
			function genarateSinglePagePDF () {
				...
				var iInnerGanttWidth = Number(this._oDomToPrint[0].style.width.slice(0,-2)) - 32;
				if (counter === 0) {
				...
				if (counter === 0 && oGanttChartSvg.getElementsByClassName("sapGanttChartVerticalLine") && oGanttChartSvg.getElementsByClassName("sapGanttChartVerticalLine").length) {
					var sVerticalLinePath = this._generateVerticalLines(this._ganttChartClone);
				...
				if (counter === 0 ) {
					...
					oSplitterBar.setAttribute("style", "color: #346187;background-color: #c9d8e7;width: 1rem;display: inline-block; height: " + this._iGanttHeight + "px");
				...
				oPdf.html(oPrintHTML, {
					callback: function (oPdf) {
						svg2pdf(mergedSVG, oPdf, {
							xOffset: ((iTableAreaWidthInPx + 16) * nScale) + fMarginRight,
							yOffset: yOffset,
							scale: nScale
						});
						counter++;
			...
			if (bShowHeaderText) {
				...
				oPdf.text(sHeaderText, this._getFXTextPosition(), fMarginTop + (this._iHeaderAndFooterHeight / 2));
			}
			...
			if (this._oModel.getProperty("/showFooterText")) {
				var sFooterText = this._oModel.getProperty("/footerText");
				nHeaderFooterHeight = nHeaderFooterHeight + this._iHeaderAndFooterHeight;
				oPdf.text(sFooterText, this._getFXTextPosition(), oPdf.getPageHeight() - fMarginBottom - (this._iHeaderAndFooterHeight / 2) -
				(this._isFooterMultiline() ? this._iHeaderAndFooterHeight : 0));
			}
			var bShrinkByWidth = this._isDomShrinkableToOnePageByWidth(oDomToPrintHeight, oDomToPrintWidth);
			var nScale = bShrinkByWidth ? (fPaperWidth   - fMarginRight - fMarginLeft) / oDomToPrintWidth : (fPaperHeight - fMarginTop - fMarginBottom - nHeaderFooterHeight) / oDomToPrintHeight;
		...
		GanttPrinting.prototype._setLinesHeight = function (oGanttChartSvg) {
			...
			if (oGanttChartSvg.getElementsByClassName("sapGanttNowLineBodySvgLine").length) {
				oGanttChartSvg.getElementsByClassName("sapGanttNowLineBodySvgLine")[0].getElementsByTagName('line')[0].setAttribute('y2', this._oDomToPrintHeight);
			}
			if (oGanttChartSvg.getElementsByClassName("sapGanttChartAdhocLine").length) {
				Array.prototype.slice.call(oGanttChartSvg.getElementsByClassName("sapGanttChartAdhocLine")[0].getElementsByTagName("line")).forEach(function (elm){
					elm.setAttribute('y2', that._oDomToPrintHeight);
				});
			}
			...
			if (oGanttChartSvg.getElementsByClassName("sapGanttChartAreaDeltaLine").length) {
				Array.prototype.slice.call(oGanttChartSvg.getElementsByClassName("sapGanttChartAreaDeltaLine")[0].getElementsByTagName("rect")).forEach(function (elm){
					elm.setAttribute('height', that._oDomToPrintHeight);
				});
			}
		};
		...
		GanttPrinting.prototype._generateVerticalLines = function (oGantt) {
			var iRenderedWidth = Number(this._oDomToPrint[0].style.width.slice(0,-2)),
			...
			return new FlexBox({
				...
				items: [
					...
					new Input({
						...
						change: fnUpdateDialog.bind(this)
					})
				]
			});
		};
		...
		GanttPrinting.prototype._createOptionsForm = function () {
			return new FlexBox({
				...
				items: [
					...
					this._createPaperComboBox(false),
					...
					this._createDurationComboBox(false),
		...
		GanttPrinting.prototype._createAndOpenDialog = function (bSinglePage) {
