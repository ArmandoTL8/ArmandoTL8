...
sap.ui.define([
		...
		if (this.getGanttChartBase().getIsConnectorDetailsVisible()){
			var fetchConnectorData = function () {
				...
				for (var i = 0; i < aElements.length; i++) {
					...
					if (aElements[i].tagName === "path" && sap.ui.getCore().byId(relationshipNode.id).isA("sap.gantt.simple.Relationship") && relationshipIdList.indexOf(relationshipNode.id) === -1 ) {
						relationshipIdList.push(relationshipNode.id);
						var relInst = sap.ui.getCore().byId(relationshipNode.id);
	...
	Relationship.prototype.renderElement = function (oRm, oElement, sGanttId) {
        ...
        if (!oElement.mRelatedShapes && !oElement.mAnchors && !GanttUtils.relationexist(this.oGantt,oElement)){
			return;
		}
