		...
		this._oSearchButton = new OverflowToolbarButton(this.getId() + "-findAndSelectSearchButton", {
			...
			press: function () {
				if (this.getParent().getShowSearchSidePanel()) {
					...
					var oSearchTextDom = oInputField.getDomRef().children[0].children[0];
					oInputField.focus();
	...
	ContainerToolbar.prototype._genSettings = function () {
		...
		this._oSettingsDialog = new ViewSettingsDialog(this.getId() + "-settingsDialog", {
            ...
            confirm: function() {
                ...
                if (this._isParentGanttContainer()) {
					var _oContainer = this.getParent();
                    if (this._percentageArray && this._percentageArray.length){
                        _oContainer.getGanttCharts().forEach(function(oGantt,index){
							_oContainer._oSplitter.getContentAreas()[index].getLayoutData().setSize(this._percentageArray[index] + "%");
                        }.bind(this));
                    } else {
                        _oContainer._oSplitter.resetContentAreasSizes();
                    }
                }
            }.bind(this),
			cancel: function() {
		...
		this._oSettingsButton = new OverflowToolbarButton(this.getId() + "-settingsButton", {
			...
			press: function (oEvent) {
                if (this._isParentGanttContainer()) {
                    this._percentageArray = [];
                    var ganttHeights = this.getParent()._oSplitter._calculatedSizes, sum = 0;
                    ganttHeights.forEach(function (elem) {
                        sum = sum + elem;
                    });
                    ganttHeights.forEach(function (sHeight) {
                        this._percentageArray.push(sHeight / sum * 100);
                    }.bind(this));
                }
