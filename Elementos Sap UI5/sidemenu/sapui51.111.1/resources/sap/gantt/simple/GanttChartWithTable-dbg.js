...
sap.ui.define([
		...
		metadata: {
			...
			properties: {
				...
				*/
				 ...
				 * @private
				 */
				_enableRTA: { type: 'boolean', defaultValue: false},
				/**
				...
				*/
				 ...
				 * @private since 1.110
				 */
				enablePseudoShapesDisplay: { type: "boolean", defaultValue: false}
			},
			aggregations: {
				/**
				...
				*/
				 ...
				 * @private
				 */
				_pseudoSvgDefs: {type: "sap.gantt.def.SvgDefs", multiple: false, singularName: "pseudoSvgDef"},
				/**
	...
	GanttChartWithTable.prototype._addChangesToControlPersonalizationWriteAPI = function(oEvent, aChanges) {
		...
		var isVariantEnabled = (isGanttChartContainer && oParent.getEnableVariantManagement()) || this.getProperty("_enableRTA");
		if (this.getEnablePseudoShapesDisplay()){
	...
	GanttChartWithTable.prototype._onTableRowsUpdated = function (oEvent) {
		...
		if (this.getEnablePseudoShapesDisplay() && this.pseudoShapeSpecificData && ((this.pseudoShapeSpecificData.oDraggedShapeDates && this.pseudoShapeSpecificData.draggedShapeTRowIndex != null) || (this.pseudoShapeSpecificData.resizedShape && this.pseudoShapeSpecificData.resizedShapeRowIndex != null))){
		...
		if (this._createInnerGanttRenderPromise()) {
	...
	GanttChartWithTable.prototype._addBackTasksToRows = function(oParams) {
		...
		if (oTaskBindingInfo) {
			...
			var needGradientCalculations = this.getEnablePseudoShapesDisplay() && ((pseudoShapeTemplate ? pseudoShapeTemplate.getTypeOfOverlapIndicator() : oBasePseudoShape.getTypeOfOverlapIndicator()) != "Indicator");
				if (!oParams) {
					...
					aRows.forEach(function(oRow) {
						...
						if (oRowBindingContext) {
							...
							if ((aExpandedIndices.indexOf(oRow.getIndex()) > -1 && !(bIncludePseudoShape)) || !this.getEnablePseudoShapesDisplay()) {
					...
					oParams.aIndices.forEach(function(index) {
						...
						if ((oParams.bExpanded && (!oParams.bPseudoExpand)) || !this.getEnablePseudoShapesDisplay()) {
	...
	GanttChartWithTable.prototype._updateVisibleHorizon = function (oTimeHorizon, sReasonCode, nVisibleWidth) {
		...
		if (sReasonCode === "initialRender") {
			...
			oAxisTimeStrategy.createAxisTime(oAxisTimeStrategy.getLocale());
		}
	 ...
	 * @private
	 */
	GanttChartWithTable.prototype._expand = function(sSchemeKey, vRowIndex,bPseudoShapeExpand) {
		if (this.getEnablePseudoShapesDisplay()){
			this._addBackTasksToRows({
				aIndices : vRowIndex,
				bExpanded: true,
		...
		if (bPseudoShapeExpand) {
			this.toggleShapeScheme(true, sSchemeKey, vRowIndex,bPseudoShapeExpand);
		} else  {
	...
	GanttChartWithTable.prototype.collapse = function(sSchemeKey, vRowIndex) {
		this._collapse(sSchemeKey,vRowIndex);
	};
	 ...
	 * @private
	 */
	GanttChartWithTable.prototype._collapse = function(sSchemeKey, vRowIndex,bPseudoShapeExpand) {
		var aRowIndex = [];
		...
		if (bPseudoShapeExpand){
			this.toggleShapeScheme(false, sSchemeKey, vRowIndex,bPseudoShapeExpand);
		} else {
	 ...
	 * @private
	 */
	GanttChartWithTable.prototype.toggleShapeScheme = function(bExpanded, sShapeScheme, vRowIndex,bPseudoShapeExpand) {
		var aIndices = [];
		...
		if (aIndices.length === 0 || (!sShapeScheme && !bPseudoShapeExpand)) { return; }

		var aExpandScheme = this.getShapeSchemes().filter(function(oScheme){
			return oScheme.getKey() === sShapeScheme;
		});

		if (!bPseudoShapeExpand && (aExpandScheme == null || aExpandScheme.length === 0 || aExpandScheme.length > 1) ){
		...
		this._bExpandToggled = this._oExpandModel.isTableRowHeightNeedChange(bExpanded, this.getTable(), aIndices, oPrimaryScheme, aExpandScheme[0]);
	...
	GanttChartWithTable.prototype.isShapeVisible = function(oShape) {
		...
		// check if locale is defined and convert from UTC to local time
		if (this.getLocale() && (oStartTime && oEndTime)) {
			oStartTime = Format._convertUTCToLocalTime(oStartTime, this.getLocale());
			oEndTime = Format._convertUTCToLocalTime(oEndTime, this.getLocale());
		}
	 ...
	 */
	 GanttChartWithTable.prototype.isCalendarTimeIntervalVisible = function(oTimeInterval) {
		...
		if (this.getLocale() && (oStartTime && oEndTime)) {
			...
			//simply convert the abap timestamp to date
