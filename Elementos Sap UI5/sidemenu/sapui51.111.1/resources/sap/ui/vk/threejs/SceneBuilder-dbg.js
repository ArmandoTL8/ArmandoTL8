...
sap.ui.define([
	...
	var SceneBuilder = function(rootNode, contentResource, resolve, reject) {
		...
		this._geometries = new Map();  // geometryId -> THREE.BufferGeometry
	...
	SceneBuilder.prototype.insertSubmesh = function(submeshInfo, sceneBoundingbox) {
		...
		if (submeshInfo.lods) {   // streaming with lods
			...
			if (geometry) {
				...
				submesh = new THREE.Mesh(innerBoxGeometry ? innerBoxGeometry : new THREE.BoxGeometry(1, 1, 1), material);
	...
	SceneBuilder.prototype.setGeometry = function(geoInfo) {
		...
		if (!geoInfo.isPolyline) {
			if (!data.normals || data.normals.length !== positionAttribute.array.length) {
