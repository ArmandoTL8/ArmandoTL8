...
sap.ui.define([
	...
	HitTester.prototype.hitTest = function(x, y, width, height, renderer, scene, camera, clippingPlanes, options) {
		...
		var rBits = Math.min(gl.getParameter(gl.RED_BITS), 8);
		var gBits = Math.min(gl.getParameter(gl.GREEN_BITS), 8);
		var bBits = Math.min(gl.getParameter(gl.BLUE_BITS), 8);
		...
		function traverseVisible(node, renderStage) {
			...
			if (geometry && node.material && frustum.intersectsObject(node)) {
				...
				if (raycaster.ray.intersectsSphere(sphere)) {
					...
					if (geometry.boundingBox == null || ray.intersectsBox(geometry.boundingBox)) {
						if (!renderStage) {
