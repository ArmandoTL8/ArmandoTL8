...
sap.ui.define([
	...
	function createMergedGeometry(node) {
		...
		if (node.isMesh && !box3IsNull(node.geometry.boundingBox) && !node.userData.skipIt) {
			var nodeGeometry = validateGeometry(node.geometry);
			if (nodeGeometry) {
		...
		for (var i = 0, l = node.children.length; i < l; i++) {
			...
			if (child.isMesh && !box3IsNull(child.geometry.boundingBox) && child.userData.skipIt) {
				var childGeometry = validateGeometry(child.geometry);
				if (childGeometry) {
					child.updateMatrix();
					childGeometry.applyMatrix4(child.matrix);
					geometry = geometry ? THREE.BufferGeometryUtils.mergeBufferGeometries([geometry, childGeometry]) : childGeometry;
				}
			}
		}
