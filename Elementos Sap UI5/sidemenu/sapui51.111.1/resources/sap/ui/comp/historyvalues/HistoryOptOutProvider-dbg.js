...
sap.ui.define([
	...
	HistoryOptOutProvider.prototype._initialize = function () {
		...
		this._oSuggestionsEnabledSwitch = null;
		this._oSuggestionsEnabledLabel = null;
			...
			oAddActionButtonProperties = {
				...
				oControlProperties: {
					...
					press: function () {
						...
						var oHistoryEnabledPromise = this._oHistoryGlobalDataService.getHistoryEnabled().then(function (bEnabled) {
							this._oHistoryEnabledSwitch.setState(bEnabled);
						}.bind(this));

						var oSuggestionsEnabledPromise = this._oHistoryGlobalDataService.getSuggestionsEnabled().then(function (bEnabled) {
							this._oSuggestionsEnabledSwitch.setState(bEnabled);
						}.bind(this));

						Promise.all([oHistoryEnabledPromise, oSuggestionsEnabledPromise]).then(function () {
							this._oDialog.open();
						}.bind(this));
					}.bind(this)
				},
	...
	HistoryOptOutProvider.prototype._createDialogContent = function () {
		...
		// this._createSuggestionsEnabledSwitch();
		this._oSuggestionsEnabledSwitch = {
			setState: function () {},
			getState: function () { return true; }
		};
		// this._createSuggestionsEnabledLabel();
	...
	HistoryOptOutProvider.prototype._createLayouts = function () {
		...
		// this._oSuggestionsEnabledLayout = new HBox({
		// 	alignItems: "Center",
		// 	items: [this._oSuggestionsEnabledLabel, this._oSuggestionsEnabledSwitch]
		// });
		this._oDialogLayout = new VBox({
			items: [this._oHistoryEnabledLayout, this._oDeleteHistoryLayout/*, this._oSuggestionsEnabledLayout*/]
	...
	HistoryOptOutProvider.prototype._createSuggestionsEnabledSwitch = function () {
		this._oSuggestionsEnabledSwitch = new Switch();
	};

	HistoryOptOutProvider.prototype._createSuggestionsEnabledLabel = function () {
		this._oSuggestionsEnabledLabel = new Label({
			text: this._oRB.getText("SUGGESTIONS_SETTING_ENABLE_DESCRIPTION")
		}).addStyleClass("sapUiSmallMarginEnd");
	};
	...
	HistoryOptOutProvider.prototype._onSavePress = function () {
		MessageBox.confirm(this._oRB.getText("HISTORY_SETTING_CONFIRM"), {
			onClose: function (sResult) {
				...
				var oHistoryEnabledPromise = this._oHistoryGlobalDataService.setHistoryEnabled(this._oHistoryEnabledSwitch.getState());
				var oSuggestionsEnabledPromise = this._oHistoryGlobalDataService.setSuggestionsEnabled(this._oSuggestionsEnabledSwitch.getState());
				Promise.all([oHistoryEnabledPromise, oSuggestionsEnabledPromise]).then(function () {
	...
	HistoryOptOutProvider.prototype.exit = function () {
		...
		this._oHistoryEnabledLayout = null;
		this._oDeleteHistoryLayout = null;
		this._oSuggestionsEnabledLayout = null;
	};
