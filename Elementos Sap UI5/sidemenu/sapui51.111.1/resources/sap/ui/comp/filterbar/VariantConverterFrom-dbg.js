 ...
 */
sap.ui.define([
	'sap/ui/comp/library', 'sap/ui/comp/util/DateTimeUtil', 'sap/m/DatePicker', 'sap/m/DateRangeSelection', "sap/base/Log", "sap/ui/core/format/DateFormat"
], function(library, DateTimeUtil, DatePicker, DateRangeSelection, Log, DateFormat) {
	"use strict";
	...
	VariantConverterFrom.prototype._createDateObject = function(sDateString/* "2015-02-28T23:00:00.000" || "20160708" || "2015-02-28T23:00:00.000 01"*/) {
		...
		if (aParts.length < 2) { // There are no Time parts if aParts length is less than 2
			oDate = new Date(aDP[0], aDP[1] - 1, aDP[2]);
		} else {
			aTP = aParts[1].split(":");
			oDate = new Date(aDP[0], aDP[1] - 1, aDP[2], aTP[0], aTP[1], aTP[2], sMillis ? sMillis : 0);
		}
