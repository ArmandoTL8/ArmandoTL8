...
sap.ui.define([
	...
	SmartTable.prototype._triggerUI5ClientExport = function(bExportAs) {
		...
		mExportSettings = {
			workbook: {
				...
				drillState: (this._isTreeTable && oRowBinding.getTreeAnnotation) ? oRowBinding.getTreeAnnotation("hierarchy-drill-state-for") : undefined,
				context: {
	...
	SmartTable.prototype._createColumnMenuContent = function(oEvent) {
		...
		if (this.getUseTablePersonalisation()) {
			...
			if (this._bIsFilterPanelEnabled && (this._isMobileTable || !this._aExistingColumns.includes(oColumnInfo.columnKey) || (oShowEntry && oShowEntry.filter))) {
