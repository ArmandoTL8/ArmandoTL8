    ...
    _createClass(MultiProvider, [{
      ...
      value: function initAsync(properties) {
        try {
          const _this6 = this;
          var _this2 = _this6;
          _this6.log = new Log("MultiProvider");
          _this6.sina = properties.sina;
          _this6.facetMode = FacetMode[properties.facetMode] || FacetMode.flat;
          _this6.federationType = FederationType[properties.federationType] || FederationType.advanced_round_robin;
          _this6.multiSina = [];
          _this6.multiDataSourceMap = {}; //key: multiId, value: originalDataSource
          _this6.sina.dataSourceMap[_this6.sina.allDataSource.id] = _this6.sina.allDataSource;
          _this6.providerHelper = new ProviderHelper(_this6);
          switch (_this6.federationType) {
            case FederationType.advanced_round_robin:
              {
                _this6.federationMethod = new FederationMethod.AdvancedRoundRobin();
                break;
              }
            case FederationType.ranking:
              {
                _this6.federationMethod = new FederationMethod.Ranking();
                break;
              }
            case FederationType.round_robin:
              {
                _this6.federationMethod = new FederationMethod.RoundRobin();
                break;
              }
          }
          _this6.sina.capabilities = _this6.sina._createCapabilities({
            fuzzy: false
          });
          ...
          return _await(Promise.allSettled(creationPromises), function (promises) {
            ...
            if (!hasSubProvider) {
              _this6.log.error("Error during creation of multi provider: no valid subproviders");
              return Promise.reject();
            }
            _this6.sina.dataSources.sort(function (a, b) {
      ...
      value: function createAsync(configuration) {
        try {
          const _this7 = this;
          _this7.log.debug("Creating new eshclient instance using provider ".concat(configuration.provider));
      ...
      value: function handleAllSearch(query) {
        try {
          ...
          var childQuery;
          var queries = [];
          var searchResultSet = _this8.initializeSearchResultSet(query);
          var searchResultSetItemList = [];

          // search with all dataSource
          searchResultSet.facets.push(_this8.sina._createDataSourceResultSet({
            title: query.filter.dataSource.label,
            items: [],
            query: query
          }));
          for (var i = 0; i < _this8.multiSina.length; i++) {
            childQuery = _this8.multiSina[i].createSearchQuery({
              calculateFacets: query.calculateFacets,
              multiSelectFacets: query.multiSelectFacets,
              dataSource: _this8.multiSina[i].allDataSource,
              searchTerm: query.getSearchTerm(),
              top: query.top,
              skip: query.skip,
              sortOrder: query.sortOrder,
              sina: _this8.multiSina[i]
            });
      ...
      value: function handleBusinessObjectSearch(query) {
        try {
          ...
          var childDataSource = _this10.multiDataSourceMap[query.filter.dataSource.id];
          var rootCondition = query.getRootCondition().clone();
          var searchResultSet = _this10.initializeSearchResultSet(query);
          _this10.providerHelper.updateRootCondition(rootCondition, childDataSource.sina);
      ...
      value: function executeSuggestionQuery(query) {
        ...
        if (query.filter.dataSource === that.sina.allDataSource) {
          ...
          return Promise.allSettled(queries).then(function (results) {
        ...
        // dataSource UserCategory (My Favorites)
        if (this.getFilterDataSourceType(query.filter.dataSource) === FilterDataSourceType.UserCategory) {
          var emptySuggestionResultSet = that.sina._createSuggestionResultSet({
